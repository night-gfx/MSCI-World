...
const PARAM_IDS = ["showRegression","regShort","regMedium","regLong","showBollinger","bollingerWindow","bollingerStd","showKalman","kalmanQ","kalmanR","showWhittaker","smoothLambda","showWhittakerRegression","whittakerRegressionGap"];
...
function renderTools(){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      const regressions=$("showWhittakerRegression").checked?whittakerTrendRegressions(analysisPoints,fullSmooth,+$("whittakerRegressionGap").value||0):null;
      if(regressions?.phases?.length){
        ...
        regressions.phases.forEach((phase,phaseIndex)=>{
          ...
          const indices=Array.from({length:endIndex-startIndex+1},(_,offset)=>startIndex+offset),x=indices.map(index=>new Date(dailyFull[index][0])),actual=indices.map(index=>fullSmooth[index]),regressionValues=indices.map(index=>phase.valueAtIndex(index)),residualAbs=indices.map((index,j)=>Number.isFinite(actual[j])&&Number.isFinite(regressionValues[j])?actual[j]-regressionValues[j]:null),residualPct=indices.map((index,j)=>Number.isFinite(residualAbs[j])&&regressionValues[j]!==0?residualAbs[j]/regressionValues[j]*100:null);
          visibleSegments.push({phase,phaseIndex:visibleSegments.length+1,indices,x,actual,regression:regressionValues,residualAbs,residualPct});
        });
        if(visibleSegments.length){panels.push({kind:"whittaker-phase-regression",label:"Whittaker-Phasen & lineare Regression",color:"#0891b2",segments:visibleSegments});panels.push({kind:"whittaker-phase-residual",label:"Whittaker − Phasenregression (%)",color:"#0891b2",segments:visibleSegments});}
        const active=regressions.active;
        const visiblePhaseRows=regressions.phases.filter(phase=>phase.displayEnd>=dailyStart&&phase.start<dailyFull.length).map((phase,index)=>{
          const startDate=analysisPoints[phase.start][0],endDate=analysisPoints[Math.min(phase.displayEnd,analysisPoints.length-1)][0],sign=phase.slope>=0?"+":"−",activeTag=phase.completed?"":" · aktuell",fitNote=!phase.completed&&regressions.gap?` · Fit bis ${fmtDate(analysisPoints[phase.end][0])}`:"";
          return `<div class="whittaker-phase-row"><span class="whittaker-phase-swatch" style="background:${phase.color}"></span><span><b>Phase ${index+1}${activeTag}</b><br>${fmtDate(startDate)} – ${fmtDate(endDate)}${fitNote}</span><code>ŷ = ${fmt(phase.intercept,4)} ${sign} ${fmt(Math.abs(phase.slope),4)}·t</code><span>R² ${fmt(phase.r2,3)}</span></div>`;
        }).join("");
        if(active){
          const latestIndex=regressions.analysisIndex,currentFit=active.valueAtIndex(latestIndex),currentActual=fullSmooth[latestIndex],currentAbs=currentActual-currentFit,currentPct=currentFit?currentAbs/currentFit*100:null;
          $("whittakerRegressionStatus").innerHTML=`<strong>Phasenregressionen auf dem Whittaker–Eilers-Filter</strong><span>Abgeschlossene Phasen: Regression über alle Filterwerte zwischen Hoch- und Tiefpunkt. Aktuelle Phase: die letzten ${regressions.gap} Handelstage werden nicht geschätzt, sondern nur mit der zuvor geschätzten Regression fortgeführt.</span><span>Aktuelle Abweichung Whittaker − Regression: <b class="${currentPct>=0?"positive":"negative"}">${currentPct>=0?"+":""}${fmt(currentPct,2)} % (${currentAbs>=0?"+":""}${fmt(currentAbs,4)})</b></span><span>Steigung der aktiven Regression: <b class="${active.slope>=0?"positive":"negative"}">${active.slope>=0?"+":""}${fmt(active.slope,4)} pro Handelstag</b></span><div class="whittaker-phase-list">${visiblePhaseRows}</div><span class="whittaker-phase-equation-note">t = Handelstage seit Beginn der jeweiligen Phase.</span>`;
        }else{
          $("whittakerRegressionStatus").innerHTML=`<strong>Phasenregressionen auf dem Whittaker–Eilers-Filter</strong><span>Abgeschlossene Phasen werden vollständig regressiert. Die aktuelle Phase ist noch zu kurz, um nach Ausschluss der letzten ${regressions.gap} Handelstage mindestens zwei Beobachtungen zu schätzen.</span><div class="whittaker-phase-list">${visiblePhaseRows}</div><span class="whittaker-phase-equation-note">t = Handelstage seit Beginn der jeweiligen Phase.</span>`;
        }
      }else if($("showWhittakerRegression").checked){
        $("whittakerRegressionStatus").innerHTML="<strong>Phasenregressionen</strong><span>Noch keine vollständige Hoch-/Tiefpunkt-Phase im verfügbaren Filter erkannt.</span>";
      }else{
        $("whittakerRegressionStatus").innerHTML="<strong>Aktueller Stand</strong><span>Der letzte Whittaker-Punkt ist exakt am neuesten verfügbaren Kurs verankert.</span><span>Phasenregressionen sind deaktiviert.</span>";
      }
    }else{$("whittakerRegressionStatus").textContent="Für den Whittaker–Eilers-Smoother liegen zu wenige Daten vor.";}
  }else if($("whittakerRegressionStatus"))$("whittakerRegressionStatus").textContent="Whittaker aktivieren, um Phasenregressionen anzuzeigen.";
  ...
  panels.forEach((panel,index)=>{
    ...
    if(isPhaseRegression){
      for(const segment of panel.segments){
        const phase=segment.phase,color=phase.color,label=`Phase ${segment.phaseIndex}${phase.completed?"":" · aktuell"}`;
        traces.push({...lineTrace(segment.x,segment.actual,`${label} Whittaker`,color,"solid",axis,1.35),showlegend:false,opacity:.68,hovertemplate:`${label}<br>Whittaker: %{y:.4f}<extra></extra>`});
        if(!phase.completed&&phase.end<phase.displayEnd){
          const fitPositions=segment.indices.map((value,pos)=>value<=phase.end?pos:-1).filter(pos=>pos>=0),extensionPositions=segment.indices.map((value,pos)=>value>=phase.end?pos:-1).filter(pos=>pos>=0);
          if(fitPositions.length>=2){const x=fitPositions.map(pos=>segment.x[pos]),y=fitPositions.map(pos=>segment.regression[pos]);traces.push({...lineTrace(x,y,`${label} Regression`,color,"dash",axis,2.4),showlegend:false,hovertemplate:`${label}<br>Regression: %{y:.4f}<extra></extra>`});}
          if(extensionPositions.length>=2){const x=extensionPositions.map(pos=>segment.x[pos]),y=extensionPositions.map(pos=>segment.regression[pos]);traces.push({...lineTrace(x,y,`${label} Fortführung`,color,"dot",axis,2.5),showlegend:false,hovertemplate:`${label} · fortgeführt<br>Regression: %{y:.4f}<extra></extra>`});}
        }else traces.push({...lineTrace(segment.x,segment.regression,`${label} Regression`,color,"dash",axis,2.35),showlegend:false,hovertemplate:`${label}<br>Regression: %{y:.4f}<extra></extra>`});
      }
    }else if(isPhaseResidual){
      for(const segment of panel.segments){
        const phase=segment.phase,color=phase.color,label=`Phase ${segment.phaseIndex}${phase.completed?"":" · aktuell"}`;
        traces.push({...lineTrace(segment.x,segment.residualPct,`${label} Abweichung`,color,"solid",axis,1.8),showlegend:false,fill:"tozeroy",fillcolor:phase.direction>0?"rgba(22,163,74,.09)":"rgba(220,38,38,.08)",customdata:segment.residualAbs,hovertemplate:`${label}<br>Whittaker − Regression: %{y:+.2f} %<br>Absolut: %{customdata:+.4f}<extra></extra>`});
      }
...
function backtestSignals(rows,type,calculationStart){const points=backtestSignalPoints(rows,type),signals=Array(rows.length).fill(null),filter=Array(rows.length).fill(null);
  ...
  const window=Math.max(10,+$("btWhittakerWindow").value||250),lambda=Math.max(.1,+$("btWhittakerLambda").value||1000);for(let i=Math.max(2,calculationStart);i<points.length;i++){const sample=points.slice(Math.max(0,i-window+1),i+1),smooth=whittakerEilersStandard(sample,lambda);filter[i]=smooth.at(-1);signals[i]=filterSlope(smooth,smooth.length-1);}return {signals,filter};
}
...
function syncBacktestControls(){const type=btStrategyValue();document.querySelectorAll(".strategy-settings").forEach(card=>card.classList.toggle("hidden",card.dataset.strategy!==type));$("btStrategyCard").classList.remove("regression-card","bollinger-card","kalman-card","whittaker-card");$("btStrategyCard").classList.add(`${type}-card`);const key=instrumentKey();if(spreadInstrument!==key){$("btSpread").value=DEFAULT_SPREADS[key]??10;spreadInstrument=key;}
  const isBollinger=type==="bollinger";$("btConditionLabel").textContent=isBollinger?"%B (Position im Band)":"Filtersteigung in %";
...
function renderBacktest(){
  ...
  const type=btStrategyValue(),showSlope=type==="whittaker",color=type==="regression"?"#7c3aed":type==="bollinger"?"#2563eb":type==="kalman"?"#db2777":"#0891b2",cutoff=rangeStart(rows.at(-1)[0],backtestRange),start=Math.max(1,rows.findIndex(row=>row[0]>=cutoff)),window=type==="regression"?Math.max(3,+$("btRegressionWindow").value||182):type==="whittaker"?Math.max(10,+$("btWhittakerWindow").value||250):type==="bollinger"?Math.max(2,+$("btBollingerWindow").value||20):20,calculationStart=Math.max(1,start-window-2),computed=backtestSignals(rows,type,calculationStart),result=runBacktest(rows,computed.signals,type,start);backtestResults={[type]:result};
  const visible=rows.slice(start),signalPoints=backtestSignalPoints(rows,type).slice(start),dates=visible.map(row=>new Date(row[0])),prices=signalPoints.map(point=>point[1]),filter=computed.filter.slice(start),signalBars=computed.signals.slice(start),bandExtras=type==="bollinger"?[computed.bands.upper.slice(start),computed.bands.lower.slice(start)]:[],priceLabel=type==="whittaker"?"Last Price / Tages-Close":"Adjusted Close",traces=[{...lineTrace(dates,prices,priceLabel,"#0f172a","solid",1,1.8),hovertemplate:`${priceLabel}: %{y:.4f}<extra></extra>`}];
  if(type==="bollinger")traces.push({...lineTrace(dates,bandExtras[0],"Bollinger Upper","#60a5fa","dot",1,1.2),hoverinfo:"skip"},{...lineTrace(dates,bandExtras[1],"Bollinger Lower","#60a5fa","dot",1,1.2),hoverinfo:"skip",fill:"tonexty",fillcolor:"rgba(59,130,246,.08)"},{...lineTrace(dates,filter,"Bollinger Mittelwert",color,"solid",1,1.4),hoverinfo:"skip"});else traces.push({...lineTrace(dates,filter,showSlope?"Whittaker–Eilers · Walk-forward":backtestName(type),color,"solid",1,2.2),hovertemplate:"Filterstand: %{y:.4f}<extra></extra>"});
  ...
  const initial=Math.max(10,+$("btCapital").value||10000),spread=Math.max(0,+$("btSpread").value||0)/20000,firstAsk=visible[0][1]*(1+spread),buyShares=(initial-1)/firstAsk,buyHold=visible.map(row=>[row[0],buyShares*row[4]]),wealthAxis=showSlope?3:2;
  if(showSlope)traces.push({x:dates,y:signalBars,type:"bar",name:"Filtersteigung",showlegend:false,xaxis:"x2",yaxis:"y2",marker:{color:signalBars.map(value=>Number.isFinite(value)&&value>=0?"#16a34a":"#dc2626")},hovertemplate:"Filtersteigung: %{y:.4f} %<extra></extra>"});
  ...
  const height=showSlope?820:650,priceDomain=showSlope?[.52,1]:[.38,1],wealthDomain=showSlope?[0,.23]:[0,.27],wealthAnnotationY=showSlope?.26:.3,layout={...baseLayout(),height,showlegend:true,bargap:.08,margin:{l:62,r:24,t:58,b:42},legend:{orientation:"h",x:0,y:1.07},xaxis:{...axisBase(signalPoints),domain:[0,1],anchor:"y",showticklabels:false,hoverformat:"%d.%m.%Y"},yaxis:{domain:priceDomain,range:paddedRange([prices,filter,...bandExtras]),tickformat:".3f",showgrid:true,gridcolor:"#f1f5f9"},annotations:[{xref:"paper",yref:"paper",x:0,y:1.02,text:`<b>${showSlope?"Last Price, Whittaker–Eilers-Filter und Ausführungen":"Kurs, damaliger Filterstand und Ausführungen"}</b>`,showarrow:false,xanchor:"left"},{xref:"paper",yref:"paper",x:0,y:wealthAnnotationY,text:"<b>Vermögensentwicklung</b>",showarrow:false,xanchor:"left"}],shapes:[]};
  if(showSlope){layout.xaxis2={...axisBase(signalPoints),domain:[0,1],anchor:"y2",matches:"x",showticklabels:false,hoverformat:"%d.%m.%Y"};layout.yaxis2={domain:[.31,.45],range:paddedRange([signalBars],true),ticksuffix:" %",tickformat:".3f",showgrid:true,gridcolor:"#f8fafc",zeroline:false};layout.annotations.push({xref:"paper",yref:"paper",x:0,y:.47,text:"<b>Filtersteigung zum Vortag</b>",showarrow:false,xanchor:"left"});layout.shapes.push({type:"line",xref:"paper",x0:0,x1:1,yref:"y2",y0:0,y1:0,line:{color:"#64748b",width:1}},{type:"line",xref:"paper",x0:0,x1:1,yref:"y2",y0:+$("btBuyValue").value||0,y1:+$("btBuyValue").value||0,line:{color:"#16a34a",width:1,dash:"dot"}},{type:"line",xref:"paper",x0:0,x1:1,yref:"y2",y0:+$("btSellValue").value||0,y1:+$("btSellValue").value||0,line:{color:"#dc2626",width:1,dash:"dot"}});layout.xaxis3={...axisBase(visible.map(row=>[row[0],row[4]])),domain:[0,1],anchor:"y3",matches:"x",hoverformat:"%d.%m.%Y"};layout.yaxis3={domain:wealthDomain,range:paddedRange([pointPrices(result.curve),pointPrices(buyHold)]),tickformat:",.0f",ticksuffix:" €",showgrid:true,gridcolor:"#f1f5f9"};}else{layout.xaxis2={...axisBase(visible.map(row=>[row[0],row[4]])),domain:[0,1],anchor:"y2",matches:"x",hoverformat:"%d.%m.%Y"};layout.yaxis2={domain:wealthDomain,range:paddedRange([pointPrices(result.curve),pointPrices(buyHold)]),tickformat:",.0f",ticksuffix:" €",showgrid:true,gridcolor:"#f1f5f9"};}
  $("backtestChart").style.height=`${height}px`;$("backtestChart").style.minHeight=`${height}px`;Plotly.react("backtestChart",traces,layout,PLOT_CONFIG).then(()=>{const graph=plotlyGraph("backtestChart");if(graph)graph.__msciZeroCenteredAxes=showSlope?["yaxis2"]:[];installVisibleYAutoscale("backtestChart");});
