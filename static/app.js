...
function calculateLatchedWhittakerWalkForward(points,endIndex,{window=250,lambda=1000,holdout=10,extremeWindow=25,extremePct=65}={}){
  ...
  for(let i=2;i<=Math.min(endIndex,points.length-1);i++){
    ...
    if(state){
      endpoints[i]=state.whittakerEndpoint;features[i]=whittakerMlFeature(points,endpoints,i);if(!features[i]||i<outOfSampleStart){phaseState[i]=0;continue;}const candidates=[];for(let j=60;j<=trainingEnd;j++){if(!features[j]||!teacherPhase[j])continue;let distance=0;for(let f=0;f<features[i].length;f++)distance+=(features[i][f]-features[j][f])**2;candidates.push({distance,positiveLabel:teacherPhase[j]>0,negativeLabel:teacherPhase[j]<0});}if(candidates.length<minTrainingExamples){phaseState[i]=0;continue;}candidates.sort((a,b)=>a.distance-b.distance);const nearest=candidates.slice(0,neighbors),weight=nearest.reduce((s,n)=>s+1/(Math.sqrt(n.distance)+1e-6),0);if(!nearest.length||!weight){phaseState[i]=latchedState;continue;}const pLow=nearest.reduce((s,n)=>s+(n.positiveLabel?1/(Math.sqrt(n.distance)+1e-6):0),0)/weight,pHigh=nearest.reduce((s,n)=>s+(n.negativeLabel?1/(Math.sqrt(n.distance)+1e-6):0),0)/weight;state.riseFromLowPct=pLow*100;state.dropFromHighPct=-pHigh*100;state.positiveEnough=pLow>=threshold&&pLow>pHigh;state.negativeEnough=pHigh>=threshold&&pHigh>pLow;state.positiveConfirmed=state.positiveEnough;state.negativeConfirmed=state.negativeEnough;state.positiveEligible=state.positiveEnough;state.negativeEligible=state.negativeEnough;state.eligible=state.positiveEligible;state.confirmed=state.positiveEnough||state.negativeEnough;
  ...
  return {states,endpoints,regime,phaseState,activation,deactivation,rawEligible,rawNegativeEligible,outOfSampleStart,trainingEnd,teacherSmooth,teacherPhase};
}
...
function renderTools(skipEmbeddedBacktest=false){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      panels.push({kind:"whittaker-extreme",label:`ML-Phasenwahrscheinlichkeit (k = ${extremeWindow})`,color:"#0891b2",bar:walk.states.slice(dailyStart).map((state,i)=>{const direction=walk.phaseState[dailyStart+i];return direction>0?state?.riseFromLowPct??null:direction<0?state?.dropFromHighPct??null:null;})});
    }
  }
...
function renderWhittakerToolsBacktest(){
  ...
  if($("wtBacktestEntryRule"))$("wtBacktestEntryRule").textContent=`GRÜN: ML P(Grün) ≥ ${fmt(computed.extremePct,0)} % → Kauf nächster Open`;
  if($("wtBacktestRegimeRule"))$("wtBacktestRegimeRule").textContent=`ROT: ML P(Rot) ≥ ${fmt(computed.extremePct,0)} % → Verkauf nächster Open`;
