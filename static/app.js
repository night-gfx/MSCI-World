...
const WT_BT_IDS = ["wtBtWindow","wtBtPositiveConfirmDays","wtBtPositiveMinPct","wtBtNegativeConfirmDays","wtBtNegativeMinPct","wtBtCapital","wtBtSpread","wtBtRange","wtBtBuyOperator","wtBtBuyValue","wtBtSellOperator","wtBtSellValue"];
...
function strictWhittakerWalkForwardState(points,index,{window=250,lambda=1000,holdout=10,positiveConfirmDays=5,positiveMinPct=1,negativeConfirmDays=5,negativeMinPct=1}={}){
  ...
  const phaseModel=linearFitFiltered(sample,smooth,phaseStart,analysisEnd),direction=phaseModel?.slope>=0?1:-1,phaseAge=analysisEnd-phaseStart+1,phaseStartValue=smooth[phaseStart],phaseEndValue=smooth[analysisEnd],phaseGainPct=Number.isFinite(phaseStartValue)&&phaseStartValue!==0?(phaseEndValue/phaseStartValue-1)*100:null;
  const positiveDays=Math.max(1,Math.floor(positiveConfirmDays)),negativeDays=Math.max(1,Math.floor(negativeConfirmDays)),positiveThreshold=Math.max(0,+positiveMinPct||0),negativeThreshold=Math.max(0,+negativeMinPct||0),positiveConfirmed=phaseAge>=positiveDays,negativeConfirmed=phaseAge>=negativeDays,positiveEnough=direction>0&&Number.isFinite(phaseGainPct)&&phaseGainPct>=positiveThreshold,negativeEnough=direction<0&&Number.isFinite(phaseGainPct)&&phaseGainPct<=-negativeThreshold,positiveEligible=positiveConfirmed&&positiveEnough,negativeEligible=negativeConfirmed&&negativeEnough,eligible=positiveEligible,confirmed=direction>0?positiveConfirmed:negativeConfirmed;
  ...
  return {index,first,sample,smooth,analysisEnd,pivots,phaseStart,phaseStartGlobal:first+phaseStart,phaseModel,model,fitEnd,fitGlobalEnd:model?first+fitEnd:null,estimate,direction,phaseAge,phaseGainPct,confirmed,positiveConfirmed,negativeConfirmed,positiveEnough,negativeEnough,positiveEligible,negativeEligible,eligible,signal};
}
function calculateLatchedWhittakerWalkForward(points,endIndex,{window=250,lambda=1000,holdout=10,positiveConfirmDays=5,positiveMinPct=1,negativeConfirmDays=5,negativeMinPct=1}={}){
  ...
  for(let i=2;i<=Math.min(endIndex,points.length-1);i++){
    const state=strictWhittakerWalkForwardState(points,i,{window,lambda,holdout,positiveConfirmDays,positiveMinPct,negativeConfirmDays,negativeMinPct});states[i]=state;
    if(state){
...
function renderWhittakerSimulationGraph(){
  ...
  syncWhittakerSimulationControls(points);const slider=$("whittakerSimSlider"),max=points.length-1,min=+slider.min||2,index=Math.max(min,Math.min(max,+slider.value||max)),snapshot=points.slice(0,index+1),lambda=Math.max(.1,+$("smoothLambda").value||1000),window=Math.max(20,+$("wtBtWindow").value||250),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||5)),positiveMinPct=Math.max(0,+$("wtBtPositiveMinPct").value||0),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||5)),negativeMinPct=Math.max(0,+$("wtBtNegativeMinPct").value||0),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),smooth=whittakerEilers(snapshot,lambda),asOf=snapshot.at(-1)[0],rangeCutoff=toolRange==="max"?-Infinity:rangeStart(asOf,toolRange),displayStart=Math.max(0,snapshot.findIndex(point=>point[0]>=rangeCutoff)),x=pointDates(snapshot.slice(displayStart)),price=pointPrices(snapshot.slice(displayStart)),smoothVisible=smooth.slice(displayStart),walk=calculateLatchedWhittakerWalkForward(points,index,{window,lambda,holdout,positiveConfirmDays,positiveMinPct,negativeConfirmDays,negativeMinPct}),phaseState=walk.phaseState,states=walk.states,activation=walk.activation,deactivation=walk.deactivation;
  const phaseAge=states.map(state=>state?.phaseAge??null),phaseGain=states.map(state=>state?.phaseGainPct??null),stateVisible=phaseState.slice(displayStart,index+1),neutral=smoothVisible.map((v,i)=>stateVisible[i]===0?v:null),green=smoothVisible.map((v,i)=>stateVisible[i]>0?v:null),red=smoothVisible.map((v,i)=>stateVisible[i]<0?v:null),custom=x.map((_,i)=>[phaseAge[displayStart+i],phaseGain[displayStart+i]]),traces=[{...lineTrace(x,price,"Tatsächlicher Kurs","#0f172a","solid",1,1.7),hovertemplate:"Kurs: %{y:.4f}<extra></extra>"},{...lineTrace(x,neutral,"Neutral","#94a3b8","solid",1,2.6),connectgaps:false,customdata:custom,hovertemplate:"Neutral<br>Whittaker: %{y:.4f}<br>Phasenalter: %{customdata[0]} Tage<br>Phasenbewegung: %{customdata[1]:+.2f} %<extra></extra>"},{...lineTrace(x,green,"Positive Phase · GRÜN","#16a34a","solid",1,3),connectgaps:false,customdata:custom,hovertemplate:"GRÜN<br>Whittaker: %{y:.4f}<br>Phasenalter: %{customdata[0]} Tage<br>Phasenbewegung: %{customdata[1]:+.2f} %<extra></extra>"},{...lineTrace(x,red,"Negative Phase · ROT","#dc2626","solid",1,3),connectgaps:false,customdata:custom,hovertemplate:"ROT<br>Whittaker: %{y:.4f}<br>Phasenalter: %{customdata[0]} Tage<br>Phasenbewegung: %{customdata[1]:+.2f} %<extra></extra>"}];
  const addSwitchMarkers=(flags,name,color,symbol)=>{const indexes=flags.map((value,i)=>value&&i>=displayStart&&i<=index?i:-1).filter(i=>i>=0);if(indexes.length)traces.push({x:indexes.map(i=>new Date(points[i][0])),y:indexes.map(i=>states[i]?.smooth?.at(-1)??points[i][1]),type:"scatter",mode:"markers",name,marker:{symbol,size:10,color,line:{color:"#fff",width:1}},customdata:indexes.map(i=>[phaseAge[i],phaseGain[i]]),hovertemplate:`${name}<br>Alter: %{customdata[0]} Tage<br>Bewegung: %{customdata[1]:+.2f} %<extra></extra>`});};
...
function renderTools(skipEmbeddedBacktest=false){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      if($("wtCurrentWhittaker"))$("wtCurrentWhittaker").textContent=fmt(fullSmooth.at(-1),4);const currentDirection=fullSmooth.length>1&&fullSmooth.at(-1)>=fullSmooth.at(-2)?1:-1;if($("wtCurrentPhase")){$("wtCurrentPhase").textContent=currentDirection>0?"Positiv":"Negativ";$("wtCurrentPhase").className=currentDirection>0?"phase-positive":"phase-negative";}
      ...
      if(regressions?.active&&$("wtCurrentPhase")){const currentPhaseDirection=regressions.active.direction;$("wtCurrentPhase").textContent=currentPhaseDirection>0?"Positiv":"Negativ";$("wtCurrentPhase").className=currentPhaseDirection>0?"phase-positive":"phase-negative";}
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  const points=backtestSignalPoints(rows,"whittaker"),window=Math.max(20,+$("wtBtWindow").value||250),lambda=Math.max(.1,+$("smoothLambda").value||1000),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||5)),positiveMinPct=Math.max(0,+$("wtBtPositiveMinPct").value||0),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||5)),negativeMinPct=Math.max(0,+$("wtBtNegativeMinPct").value||0),signals=Array(rows.length).fill(null),phaseRegression=Array(rows.length).fill(null),phaseDirection=Array(rows.length).fill(null),phaseAge=Array(rows.length).fill(null),phaseGainPct=Array(rows.length).fill(null),confirmed=Array(rows.length).fill(false),eligible=Array(rows.length).fill(false),phaseState=Array(rows.length).fill(0),rawEligible=Array(rows.length).fill(false),rawNegativeEligible=Array(rows.length).fill(false),activation=Array(rows.length).fill(false),deactivation=Array(rows.length).fill(false),walk=calculateLatchedWhittakerWalkForward(points,points.length-1,{window,lambda,holdout,positiveConfirmDays,positiveMinPct,negativeConfirmDays,negativeMinPct});
  for(let i=Math.max(2,calculationStart);i<points.length;i++){
    const state=walk.states[i];if(!state)continue;signals[i]=state.signal;phaseRegression[i]=state.estimate;phaseDirection[i]=state.direction;phaseAge[i]=state.phaseAge;phaseGainPct[i]=state.phaseGainPct;confirmed[i]=state.confirmed;rawEligible[i]=state.positiveEligible;rawNegativeEligible[i]=state.negativeEligible;eligible[i]=walk.regime[i];phaseState[i]=walk.phaseState[i];activation[i]=walk.activation[i];deactivation[i]=walk.deactivation[i];
  }
  return {points,signals,phaseRegression,phaseDirection,phaseAge,phaseGainPct,confirmed,eligible,phaseState,rawEligible,rawNegativeEligible,activation,deactivation,window,lambda,holdout,positiveConfirmDays,positiveMinPct,negativeConfirmDays,negativeMinPct};
}
function renderWhittakerToolsBacktest(){
  ...
  const range=$("wtBtRange").value||"5y",cutoff=rangeStart(rows.at(-1)[0],range),start=Math.max(1,rows.findIndex(row=>row[0]>=cutoff)),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||5)),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||5)),maxConfirmDays=Math.max(positiveConfirmDays,negativeConfirmDays),calculationStart=Math.max(2,start-window-holdout-maxConfirmDays-5),computed=calculateWhittakerToolsWalkForward(rows,calculationStart),result=runWhittakerLongOnlyBacktest(rows,computed,start);
  ...
  const visible=rows.slice(start),dates=visible.map(row=>new Date(row[0])),signalPoints=computed.points.slice(start),prices=signalPoints.map(point=>point[1]),regression=computed.phaseRegression.slice(start),direction=computed.phaseDirection.slice(start),eligible=computed.eligible.slice(start),phaseState=computed.phaseState.slice(start),phaseAge=computed.phaseAge.slice(start),phaseGain=computed.phaseGainPct.slice(start),signals=computed.signals.slice(start),greenRegression=regression.map((value,i)=>phaseState[i]>0?value:null),redRegression=regression.map((value,i)=>phaseState[i]<0?value:null),eligibleSignals=signals.map((value,i)=>eligible[i]?value:null),traces=[{...lineTrace(dates,prices,"Last Price / Tages-Close","#0f172a","solid",1,1.7),hovertemplate:"Kurs: %{y:.4f}<extra></extra>"},{...lineTrace(dates,greenRegression,"Bestätigte positive Phase","#16a34a","solid",1,2.4),connectgaps:false,hovertemplate:"GRÜN · Regression: %{y:.4f}<extra></extra>"},{...lineTrace(dates,redRegression,"Bestätigte negative Phase","#dc2626","solid",1,2.4),connectgaps:false,hovertemplate:"ROT · Regression: %{y:.4f}<extra></extra>"}];
