...
function strictWhittakerWalkForwardState(points,index,{window=250,lambda=1000,holdout=10,confirmDays=5,minPositivePct=1}={}){
  ...
  const projectionIndex=fitEnd+(index-fitGlobalEnd),estimate=model.valueAtIndex(projectionIndex),direction=model.slope>=0?1:-1,phaseAge=fitEnd-phaseStart+1,phaseStartValue=smooth[phaseStart],phaseEndValue=smooth[fitEnd],phaseGainPct=Number.isFinite(phaseStartValue)&&phaseStartValue!==0?(phaseEndValue/phaseStartValue-1)*100:null,confirmed=phaseAge>=Math.max(1,Math.floor(confirmDays)),positiveEnough=direction>0&&Number.isFinite(phaseGainPct)&&phaseGainPct>=Math.max(0,+minPositivePct||0),eligible=confirmed&&positiveEnough,signal=Number.isFinite(estimate)&&estimate!==0?(points[index][1]/estimate-1)*100:null;
  return {index,fitGlobalEnd,first,fitEnd,fitSample,smooth,pivots,phaseStart,phaseStartGlobal:first+phaseStart,model,estimate,direction,phaseAge,phaseGainPct,confirmed,positiveEnough,eligible,signal};
}
...
function renderWhittakerSimulationGraph(){
  const lab=$("whittakerWalkForwardLab"),chart=$("whittakerSimulationChart");if(!lab||!chart||!payload)return;const active=$("showWhittaker").checked;lab.classList.toggle("hidden",!active);if(!active)return;
  ...
  syncWhittakerSimulationControls(points);const slider=$("whittakerSimSlider"),max=points.length-1,min=+slider.min||2,index=Math.max(min,Math.min(max,+slider.value||max)),snapshot=points.slice(0,index+1),lambda=Math.max(.1,+$("smoothLambda").value||1000),confirmDays=Math.max(1,Math.floor(+$("wtBtConfirmDays").value||5)),minPositivePct=Math.max(0,+$("wtBtMinPositivePct").value||0),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),smooth=whittakerEilers(snapshot,lambda),displayStart=Math.min(index,Math.max(0,min)),x=pointDates(snapshot.slice(displayStart)),price=pointPrices(snapshot.slice(displayStart)),smoothVisible=smooth.slice(displayStart),eligible=Array(snapshot.length).fill(false),phaseAge=Array(snapshot.length).fill(null),phaseGain=Array(snapshot.length).fill(null);
  const pivots=whittakerTurningPoints(smooth,smooth.length-1),bounds=[0,...pivots.filter((v,i,a)=>v>0&&v<smooth.length-1&&a.indexOf(v)===i),smooth.length-1].sort((a,b)=>a-b);
  for(let b=0;b<bounds.length-1;b++){const start=bounds[b],end=bounds[b+1];for(let k=start;k<=end;k++){const age=k-start+1,gain=smooth[start]!==0?(smooth[k]/smooth[start]-1)*100:null;phaseAge[k]=age;phaseGain[k]=gain;eligible[k]=age>=confirmDays&&Number.isFinite(gain)&&gain>=minPositivePct;}}
  const gray=smoothVisible,green=smoothVisible.map((v,i)=>eligible[displayStart+i]?v:null),traces=[{...lineTrace(x,price,"Tatsächlicher Kurs","#0f172a","solid",1,1.7),hovertemplate:"Kurs: %{y:.4f}<extra></extra>"},{...lineTrace(x,gray,"Whittaker · noch nicht bestätigt","#94a3b8","solid",1,2.5),customdata:x.map((_,i)=>[phaseAge[displayStart+i],phaseGain[displayStart+i]]),hovertemplate:"Whittaker: %{y:.4f}<br>Phasenalter: %{customdata[0]} Tage<br>Phasenanstieg: %{customdata[1]:+.2f} %<extra></extra>"},{...lineTrace(x,green,"Bestätigte positive Phase","#16a34a","solid",1,3.1),connectgaps:false,customdata:x.map((_,i)=>[phaseAge[displayStart+i],phaseGain[displayStart+i]]),hovertemplate:"Bestätigte Phase<br>Whittaker: %{y:.4f}<br>Alter: %{customdata[0]} Tage<br>Anstieg: %{customdata[1]:+.2f} %<extra></extra>"}];
  const regressions=whittakerTrendRegressions(snapshot,smooth,holdout);if(regressions?.phases?.length){for(const phase of regressions.phases){const start=Math.max(phase.start,displayStart),end=Math.min(phase.displayEnd,index);if(end<start)continue;const xx=[],yy=[],colors=[];for(let k=start;k<=end;k++){xx.push(new Date(snapshot[k][0]));yy.push(phase.valueAtIndex(k));colors.push(eligible[k]);}const grayReg=yy.map((v,i)=>colors[i]?null:v),greenReg=yy.map((v,i)=>colors[i]?v:null);traces.push({...lineTrace(xx,grayReg,"Phasenregression · unbestätigt","#cbd5e1","solid",1,1.4),showlegend:false,connectgaps:false,hoverinfo:"skip"});traces.push({...lineTrace(xx,greenReg,"Phasenregression · bestätigt","#15803d","solid",1,1.8),showlegend:false,connectgaps:false,hoverinfo:"skip"});}}
  const asOf=snapshot.at(-1)[0],layout={...baseLayout(),height:560,showlegend:true,margin:{l:58,r:24,t:62,b:44},legend:{orientation:"h",x:0,y:1.08,font:{size:10}},xaxis:{...axisBase(snapshot.slice(displayStart)),hoverformat:"%d.%m.%Y"},yaxis:{range:paddedRange([price,smoothVisible]),tickformat:".3f",showgrid:true,gridcolor:"#f1f5f9"},annotations:[{xref:"paper",yref:"paper",x:0,y:1.03,text:`<b>Informationsstand ${fmtDate(asOf)} · grün erst ab ${confirmDays} Tagen und +${fmt(minPositivePct,1)} %</b>`,showarrow:false,xanchor:"left",font:{size:11,color:"#475569"}}]};
  chart.style.height="560px";Plotly.react("whittakerSimulationChart",traces,layout,PLOT_CONFIG).then(()=>installVisibleYAutoscale("whittakerSimulationChart"));
}
function renderWhittakerWalkForwardLab(){const lab=$("whittakerWalkForwardLab");if(!lab)return;const active=$("showWhittaker").checked;lab.classList.toggle("hidden",!active);if(!active)return;renderWhittakerSimulationGraph();renderWhittakerToolsBacktest();}
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  const points=backtestSignalPoints(rows,"whittaker"),window=Math.max(20,+$("wtBtWindow").value||250),lambda=Math.max(.1,+$("smoothLambda").value||1000),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),confirmDays=Math.max(1,Math.floor(+$("wtBtConfirmDays").value||5)),minPositivePct=Math.max(0,+$("wtBtMinPositivePct").value||0),signals=Array(rows.length).fill(null),phaseRegression=Array(rows.length).fill(null),phaseDirection=Array(rows.length).fill(null),phaseAge=Array(rows.length).fill(null),phaseGainPct=Array(rows.length).fill(null),confirmed=Array(rows.length).fill(false),eligible=Array(rows.length).fill(false);
  for(let i=Math.max(2,calculationStart);i<points.length;i++){
    const state=strictWhittakerWalkForwardState(points,i,{window,lambda,holdout,confirmDays,minPositivePct});if(!state)continue;
    signals[i]=state.signal;phaseRegression[i]=state.estimate;phaseDirection[i]=state.direction;phaseAge[i]=state.phaseAge;phaseGainPct[i]=state.phaseGainPct;confirmed[i]=state.confirmed;eligible[i]=state.eligible;
  }
  return {points,signals,phaseRegression,phaseDirection,phaseAge,phaseGainPct,confirmed,eligible,window,lambda,holdout,confirmDays,minPositivePct};
}
