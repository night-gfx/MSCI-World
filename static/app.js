...
function splitPriceAroundClose(x,y,axis,previousClose){
  ...
  traces.forEach(trace=>{trace.y=trace.y.map(value=>Number.isFinite(value)?value+previousClose:value);trace.fill="none";trace.hoverinfo="skip";trace.line.width=2.5;trace.meta="intraday-extension";trace.yaxis="y";});
  return traces;
}
...
function renderTools(){
  ...
  if(hasIntraday){const intradayDate=new Date(intradayPoints.at(-1)[0]).toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit",year:"numeric"});layout.shapes.push({name:"intraday-background",type:"rect",xref:"paper",yref:"paper",x0:.85,x1:1,y0:1-main,y1:1,fillcolor:"rgba(100,116,139,.11)",line:{width:0},layer:"below"},{name:"previous-close-reference",type:"line",xref:`x${rightAxisStart}`,yref:"y",x0:intradayRange[0],x1:intradayRange[1],y0:previousClose,y1:previousClose,line:{color:"#0f172a",width:2.5},layer:"below"});layout.annotations.push({xref:"paper",yref:"paper",x:.425,y:1.035,text:"<b>Daily Historical</b>",showarrow:false,xanchor:"center",yanchor:"bottom",font:{family:"Arial, sans-serif",size:11,color:"#475569"}},{xref:"paper",yref:"paper",x:.925,y:1.035,text:`<b>Intraday (${intradayDate})</b>`,showarrow:false,xanchor:"center",yanchor:"bottom",font:{family:"Arial, sans-serif",size:11,color:"#475569"}});}
