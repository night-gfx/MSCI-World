...
function renderWhittakerSimulationGraph(){
  ...
  const activationIndexes=activation.map((value,i)=>value&&i>=displayStart&&i<=index?i:-1).filter(i=>i>=0);if(activationIndexes.length)traces.push({x:activationIndexes.map(i=>new Date(points[i][0])),y:activationIndexes.map(i=>states[i]?.smooth?.at(-1)??points[i][1]),type:"scatter",mode:"markers",name:"Bestätigungssignal",marker:{symbol:"triangle-up",size:10,color:"#16a34a",line:{color:"#fff",width:1}},customdata:activationIndexes.map(i=>[phaseAge[i],phaseGain[i]]),hovertemplate:"Bestätigung wurde an diesem Tag fixiert<br>Alter: %{customdata[0]} Tage<br>Anstieg: %{customdata[1]:+.2f} %<extra></extra>"});
  const totalHeight=Math.max(520,mainChart?.clientHeight||parseInt(mainChart?.style?.height)||560);panel.style.height=`${totalHeight}px`;panel.style.minHeight=`${totalHeight}px`;const controls=panel.querySelector(".whittaker-simulation-topbar"),controlsHeight=controls?.offsetHeight||190,chartHeight=Math.max(300,totalHeight-controlsHeight-8),xRange=x.length?[new Date(x[0]),new Date(x.at(-1))]:undefined,layout={...baseLayout(),height:chartHeight,dragmode:"zoom",uirevision:`wt-sim-${toolRange}`,showlegend:true,margin:{l:58,r:20,t:48,b:44},legend:{orientation:"h",x:0,y:1.06,font:{size:9}},xaxis:{...axisBase(snapshot.slice(displayStart)),range:xRange,hoverformat:"%d.%m.%Y",showgrid:false},yaxis:{range:paddedRange([price,smoothVisible,grayRegression,greenRegression]),tickformat:".3f",showgrid:true,gridcolor:"#f1f5f9",zeroline:false},annotations:[{xref:"paper",yref:"paper",x:0,y:1.02,text:`<b>Informationsstand ${fmtDate(asOf)}</b>`,showarrow:false,xanchor:"left",font:{size:10,color:"#475569"}}]};
  chart.style.height=`${chartHeight}px`;chart.style.minHeight=`${chartHeight}px`;Plotly.react("whittakerSimulationChart",traces,layout,PLOT_CONFIG).then(()=>{installVisibleYAutoscale("whittakerSimulationChart");installWhittakerSimulationZoomBehavior();});
}
...
function runWhittakerLongOnlyBacktest(rows,computed,start){
  ...
  for(let i=start;i<rows.length;i++){
    ...
    if(!shares&&eligible&&compareSignal(signal,buyOperator,buyThreshold)&&cash>1){
      ...
      const negativePhase=computed.phaseDirection[signalIndex]<0,meanReversionExit=!negativePhase&&compareSignal(signal,sellOperator,sellThreshold);
      if(negativePhase||meanReversionExit){
        const bid=open*(1-spread),gross=shares*bid-1,profit=gross-basis,tax=Math.max(0,profit)*.25;cash=gross-tax;taxes+=tax;fees+=1;shares=0;basis=0;trades.push({side:"Verkauf",signalDate:rows[signalIndex][0],date:rows[i][0],price:bid,chartPrice:close,cost:1,tax,capital:cash,reason:negativePhase?"Regime-Exit: erste negative Phase":"Mean-Reversion-Exit"});
      }
    }
