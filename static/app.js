...
function strictWhittakerWalkForwardState(points,index,{window=250,lambda=1000,holdout=10,scoreWindow=20,positiveScore=.5,negativeScore=-.5}={}){
...
function calculateLatchedWhittakerWalkForward(points,endIndex,{window=250,lambda=1000,holdout=10,scoreWindow=20,positiveConfirmDays=5,positiveScore=.5,negativeConfirmDays=5,negativeScore=-.5}={}){
...
function renderWhittakerSimulationGraph(){
  ...
  syncWhittakerSimulationControls(points);const slider=$("whittakerSimSlider"),max=points.length-1,min=+slider.min||2,index=Math.max(min,Math.min(max,+slider.value||max)),snapshot=points.slice(0,index+1),lambda=Math.max(.1,+$("smoothLambda").value||1000),window=Math.max(20,+$("wtBtWindow").value||250),scoreWindow=Math.max(5,Math.floor(+$("wtBtScoreWindow").value||20)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||5)),positiveScore=Math.max(0,+$("wtBtPositiveScore").value||0),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||5)),negativeScore=Math.min(0,Number.isFinite(+$("wtBtNegativeScore").value)?+$("wtBtNegativeScore").value:-.5),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),smooth=whittakerEilers(snapshot,lambda),asOf=snapshot.at(-1)[0],rangeCutoff=toolRange==="max"?-Infinity:rangeStart(asOf,toolRange),displayStart=Math.max(0,snapshot.findIndex(point=>point[0]>=rangeCutoff)),x=pointDates(snapshot.slice(displayStart)),price=pointPrices(snapshot.slice(displayStart)),smoothVisible=smooth.slice(displayStart),walk=calculateLatchedWhittakerWalkForward(points,index,{window,lambda,holdout,scoreWindow,positiveConfirmDays,positiveScore,negativeConfirmDays,negativeScore}),phaseState=walk.phaseState,states=walk.states,activation=walk.activation,deactivation=walk.deactivation;
...
function renderTools(skipEmbeddedBacktest=false){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      const scoreWindow=Math.max(5,Math.floor(+$("wtBtScoreWindow").value||20)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||5)),positiveScore=Math.max(0,+$("wtBtPositiveScore").value||0),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||5)),negativeScore=Math.min(0,Number.isFinite(+$("wtBtNegativeScore").value)?+$("wtBtNegativeScore").value:-.5),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),walk=calculateLatchedWhittakerWalkForward(analysisPoints,analysisPoints.length-1,{window,lambda,holdout,scoreWindow,positiveConfirmDays,positiveScore,negativeConfirmDays,negativeScore}),currentState=walk.phaseState.at(-1)||0,currentScore=walk.states.at(-1)?.trendScore;
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  const points=backtestSignalPoints(rows,"whittaker"),window=Math.max(20,+$("wtBtWindow").value||250),lambda=Math.max(.1,+$("smoothLambda").value||1000),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),scoreWindow=Math.max(5,Math.floor(+$("wtBtScoreWindow").value||20)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||5)),positiveScore=Math.max(0,+$("wtBtPositiveScore").value||0),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||5)),negativeScore=Math.min(0,Number.isFinite(+$("wtBtNegativeScore").value)?+$("wtBtNegativeScore").value:-.5),signals=Array(rows.length).fill(null),phaseRegression=Array(rows.length).fill(null),phaseDirection=Array(rows.length).fill(null),phaseAge=Array(rows.length).fill(null),phaseGainPct=Array(rows.length).fill(null),trendScore=Array(rows.length).fill(null),confirmed=Array(rows.length).fill(false),eligible=Array(rows.length).fill(false),phaseState=Array(rows.length).fill(0),rawEligible=Array(rows.length).fill(false),rawNegativeEligible=Array(rows.length).fill(false),activation=Array(rows.length).fill(false),deactivation=Array(rows.length).fill(false),walk=calculateLatchedWhittakerWalkForward(points,points.length-1,{window,lambda,holdout,scoreWindow,positiveConfirmDays,positiveScore,negativeConfirmDays,negativeScore});
  for(let i=Math.max(2,calculationStart);i<points.length;i++){
