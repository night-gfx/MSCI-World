...
function renderWhittakerToolsBacktest(){
  ...
  const range=$("wtBtRange").value||"5y",cutoff=rangeStart(rows.at(-1)[0],range),start=Math.max(1,rows.findIndex(row=>row[0]>=cutoff)),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||25)),calculationStart=Math.max(2,start-window-holdout-extremeWindow-5),computed=calculateWhittakerToolsWalkForward(rows,calculationStart),relative=runWhittakerLongOnlyBacktest(rows,computed,start),absolute=runWhittakerFixedNotionalBacktest(rows,computed,start),spread=Math.max(0,+$("wtBtSpread").value||0)/20000,benchmark=buildWhittakerLongOnlyBenchmark(rows,start,relative.capital,spread);
