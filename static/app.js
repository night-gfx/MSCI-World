...
function renderTools(skipEmbeddedBacktest=false){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      const extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||3)),extremePct=Math.max(.1,+$("wtBtExtremePct").value||1.5),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),walk=calculateLatchedWhittakerWalkForward(analysisPoints,analysisPoints.length-1,{window,lambda,holdout,extremeWindow,extremePct}),currentState=walk.phaseState.at(-1)||0,current=walk.states.at(-1),currentDistance=currentState>0?current?.riseFromLowPct:current?.dropFromHighPct;
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  const points=backtestSignalPoints(rows,"whittaker"),window=Math.max(20,+$("wtBtWindow").value||250),lambda=Math.max(.1,+$("smoothLambda").value||1000),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||3)),extremePct=Math.max(.1,+$("wtBtExtremePct").value||1.5),signals=Array(rows.length).fill(null),phaseRegression=Array(rows.length).fill(null),phaseDirection=Array(rows.length).fill(null),phaseAge=Array(rows.length).fill(null),phaseGainPct=Array(rows.length).fill(null),riseFromLowPct=Array(rows.length).fill(null),dropFromHighPct=Array(rows.length).fill(null),confirmed=Array(rows.length).fill(false),eligible=Array(rows.length).fill(false),phaseState=Array(rows.length).fill(0),rawEligible=Array(rows.length).fill(false),rawNegativeEligible=Array(rows.length).fill(false),activation=Array(rows.length).fill(false),deactivation=Array(rows.length).fill(false),walk=calculateLatchedWhittakerWalkForward(points,points.length-1,{window,lambda,holdout,extremeWindow,extremePct});
  for(let i=Math.max(2,calculationStart);i<points.length;i++){
...
function renderWhittakerToolsBacktest(){
  ...
  if($("wtBacktestEntryRule"))$("wtBacktestEntryRule").textContent=`GRÜN: normierte Whittaker-Steigung ≥ +${fmt(computed.extremePct,1)} für 2 Tage → Kauf nächster Open`;
  if($("wtBacktestRegimeRule"))$("wtBacktestRegimeRule").textContent=`ROT: normierte Whittaker-Steigung ≤ −${fmt(computed.extremePct,1)} für 2 Tage → Verkauf nächster Open`;
