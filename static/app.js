...
const WT_BT_IDS = ["wtBtWindow","wtBtTrendWindow","wtBtPositiveConfirmDays","wtBtPositiveTau","wtBtNegativeConfirmDays","wtBtNegativeTau","wtBtCapital","wtBtAbsoluteAmount","wtBtSpread","wtBtRange"];
...
function mannKendallWhittakerTrend(values,endIndex,lookback=40){
  const length=Math.max(5,Math.floor(lookback)),start=endIndex-length+1;if(start<0)return null;let concordance=0,pairs=0;
  for(let i=start;i<=endIndex;i++){if(!Number.isFinite(values[i]))return null;for(let j=i+1;j<=endIndex;j++){if(!Number.isFinite(values[j]))return null;concordance+=Math.sign(values[j]-values[i]);pairs++;}}
  return pairs?{tau:concordance/pairs,start,end:endIndex,n:length}:null;
}
function strictWhittakerWalkForwardState(points,index,{window=250,lambda=1000,holdout=10,trendWindow=40,positiveTau=.1,negativeTau=-.7}={}){
  ...
  const phaseStart=Math.max(0,analysisEnd-Math.max(5,Math.floor(trendWindow))+1),phaseModel=linearFitFiltered(sample,smooth,phaseStart,analysisEnd),direction=phaseModel?.slope>=0?1:-1,phaseAge=analysisEnd-phaseStart+1,phaseStartValue=smooth[phaseStart],phaseEndValue=smooth[analysisEnd],phaseGainPct=Number.isFinite(phaseStartValue)&&phaseStartValue!==0?(phaseEndValue/phaseStartValue-1)*100:null;
  ...
  return {index,first,sample,smooth,whittakerEndpoint:smooth[analysisEnd],analysisEnd,pivots:[],phaseStart,phaseStartGlobal:first+phaseStart,phaseModel,model,fitEnd,fitGlobalEnd:model?first+fitEnd:null,estimate,direction,phaseAge,phaseGainPct,trendTau:null,positiveEnough:false,negativeEnough:false,signal};
}
function calculateLatchedWhittakerWalkForward(points,endIndex,{window=250,lambda=1000,holdout=10,trendWindow=40,positiveConfirmDays=1,positiveTau=.1,negativeConfirmDays=3,negativeTau=-.7}={}){
  const states=Array(points.length).fill(null),endpoints=Array(points.length).fill(null),regime=Array(points.length).fill(false),phaseState=Array(points.length).fill(0),activation=Array(points.length).fill(false),deactivation=Array(points.length).fill(false),rawEligible=Array(points.length).fill(false),rawNegativeEligible=Array(points.length).fill(false);if(points.length)endpoints[0]=+points[0][1];if(points.length>1)endpoints[1]=+points[1][1];let latchedState=0;
  let positiveStreak=0,negativeStreak=0;const positiveDays=Math.max(1,Math.floor(positiveConfirmDays)),negativeDays=Math.max(1,Math.floor(negativeConfirmDays)),positiveThreshold=Math.max(-1,Math.min(1,+positiveTau||0)),negativeThreshold=Math.max(-1,Math.min(1,Number.isFinite(+negativeTau)?+negativeTau:0));
  for(let i=2;i<=Math.min(endIndex,points.length-1);i++){
    const state=strictWhittakerWalkForwardState(points,i,{window,lambda,holdout,trendWindow,positiveTau,negativeTau});states[i]=state;
    if(state){
      endpoints[i]=state.whittakerEndpoint;const trend=mannKendallWhittakerTrend(endpoints,i,trendWindow);if(!trend){phaseState[i]=latchedState;regime[i]=latchedState>0;continue;}state.trendTau=trend.tau;state.direction=trend.tau>=0?1:-1;state.positiveEnough=trend.tau>=positiveThreshold;state.negativeEnough=trend.tau<=negativeThreshold;
  ...
  return {states,endpoints,regime,phaseState,activation,deactivation,rawEligible,rawNegativeEligible};
}
...
function renderWhittakerSimulationGraph(){
  ...
  syncWhittakerSimulationControls(points);const slider=$("whittakerSimSlider"),max=points.length-1,min=+slider.min||2,index=Math.max(min,Math.min(max,+slider.value||max)),snapshot=points.slice(0,index+1),lambda=Math.max(.1,+$("smoothLambda").value||1000),window=Math.max(20,+$("wtBtWindow").value||250),trendWindow=Math.max(5,Math.floor(+$("wtBtTrendWindow").value||40)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||1)),positiveTau=Math.max(0,Number.isFinite(+$("wtBtPositiveTau").value)?+$("wtBtPositiveTau").value:.1),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||3)),negativeTau=Math.min(0,Number.isFinite(+$("wtBtNegativeTau").value)?+$("wtBtNegativeTau").value:-.7),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),smooth=whittakerEilers(snapshot,lambda),asOf=snapshot.at(-1)[0],rangeCutoff=toolRange==="max"?-Infinity:rangeStart(asOf,toolRange),displayStart=Math.max(0,snapshot.findIndex(point=>point[0]>=rangeCutoff)),x=pointDates(snapshot.slice(displayStart)),price=pointPrices(snapshot.slice(displayStart)),smoothVisible=smooth.slice(displayStart),walk=calculateLatchedWhittakerWalkForward(points,index,{window,lambda,holdout,trendWindow,positiveConfirmDays,positiveTau,negativeConfirmDays,negativeTau}),phaseState=walk.phaseState,states=walk.states,activation=walk.activation,deactivation=walk.deactivation;
  const phaseAge=states.map(state=>state?.phaseAge??null),trendTaus=states.map(state=>state?.trendTau??null),stateVisible=phaseState.slice(displayStart,index+1),neutral=smoothVisible.map((v,i)=>stateVisible[i]===0?v:null),green=smoothVisible.map((v,i)=>stateVisible[i]>0?v:null),red=smoothVisible.map((v,i)=>stateVisible[i]<0?v:null),custom=x.map((_,i)=>[phaseAge[displayStart+i],trendTaus[displayStart+i]]),traces=[{...lineTrace(x,price,"Tatsächlicher Kurs","#0f172a","solid",1,1.7),hovertemplate:"Kurs: %{y:.4f}<extra></extra>"},{...lineTrace(x,neutral,"Neutral","#94a3b8","solid",1,2.6),connectgaps:false,customdata:custom,hovertemplate:"Neutral<br>Whittaker: %{y:.4f}<br>Bestätigungsserie: %{customdata[0]} Tage<br>Kendall τ: %{customdata[1]:+.2f}<extra></extra>"},{...lineTrace(x,green,"Positive Phase · GRÜN","#16a34a","solid",1,3),connectgaps:false,customdata:custom,hovertemplate:"GRÜN<br>Whittaker: %{y:.4f}<br>Bestätigungsserie: %{customdata[0]} Tage<br>Kendall τ: %{customdata[1]:+.2f}<extra></extra>"},{...lineTrace(x,red,"Negative Phase · ROT","#dc2626","solid",1,3),connectgaps:false,customdata:custom,hovertemplate:"ROT<br>Whittaker: %{y:.4f}<br>Bestätigungsserie: %{customdata[0]} Tage<br>Kendall τ: %{customdata[1]:+.2f}<extra></extra>"}];
  const addSwitchMarkers=(flags,name,color,symbol)=>{const indexes=flags.map((value,i)=>value&&i>=displayStart&&i<=index?i:-1).filter(i=>i>=0);if(indexes.length)traces.push({x:indexes.map(i=>new Date(points[i][0])),y:indexes.map(i=>states[i]?.smooth?.at(-1)??points[i][1]),type:"scatter",mode:"markers",name,marker:{symbol,size:10,color,line:{color:"#fff",width:1}},customdata:indexes.map(i=>[phaseAge[i],trendTaus[i]]),hovertemplate:`${name}<br>Bestätigungsserie: %{customdata[0]} Tage<br>Kendall τ: %{customdata[1]:+.2f}<extra></extra>`});};
...
function renderTools(skipEmbeddedBacktest=false){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      const trendWindow=Math.max(5,Math.floor(+$("wtBtTrendWindow").value||40)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||1)),positiveTau=Math.max(0,Number.isFinite(+$("wtBtPositiveTau").value)?+$("wtBtPositiveTau").value:.1),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||3)),negativeTau=Math.min(0,Number.isFinite(+$("wtBtNegativeTau").value)?+$("wtBtNegativeTau").value:-.7),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),walk=calculateLatchedWhittakerWalkForward(analysisPoints,analysisPoints.length-1,{window,lambda,holdout,trendWindow,positiveConfirmDays,positiveTau,negativeConfirmDays,negativeTau}),currentState=walk.phaseState.at(-1)||0,currentTau=walk.states.at(-1)?.trendTau;
      if($("wtCurrentPhase")){const label=currentState>0?"Positiv":currentState<0?"Negativ":"Neutral";$("wtCurrentPhase").textContent=`${label}${Number.isFinite(currentTau)?` · Kendall τ ${fmt(currentTau,2)}`:""}`;$("wtCurrentPhase").className=currentState>0?"phase-positive":currentState<0?"phase-negative":"";}
      panels.push({kind:"whittaker-kendall",label:`Kendall τ der Whittaker-Linie (${trendWindow} Tage)`,color:"#0891b2",bar:walk.states.slice(dailyStart).map(state=>state?.trendTau??null)});
    }
  }
  ...
  panels.forEach((panel,index)=>{
    ...
    }else if(panel.bar)traces.push({x:dailyX,y:panel.bar,type:"bar",name:panel.label,showlegend:false,marker:{color:panel.bar.map(v=>v>=0?"#16a34a":"#dc2626")},xaxis:`x${axis}`,yaxis:`y${axis}`,hovertemplate:panel.kind==="whittaker-kendall"?"Kendall τ: %{y:+.3f}<extra></extra>":"Steigung zum Vortag: %{y:.4f}<extra></extra>"});else panel.series.forEach((series,i)=>traces.push(...splitSigned(dailyX,series,axis,panel.series.length>1?(i?"Kurs - Lower Band":"Kurs - Upper Band"):panel.label)));
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  const points=backtestSignalPoints(rows,"whittaker"),window=Math.max(20,+$("wtBtWindow").value||250),lambda=Math.max(.1,+$("smoothLambda").value||1000),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),trendWindow=Math.max(5,Math.floor(+$("wtBtTrendWindow").value||40)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||1)),positiveTau=Math.max(0,Number.isFinite(+$("wtBtPositiveTau").value)?+$("wtBtPositiveTau").value:.1),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||3)),negativeTau=Math.min(0,Number.isFinite(+$("wtBtNegativeTau").value)?+$("wtBtNegativeTau").value:-.7),signals=Array(rows.length).fill(null),phaseRegression=Array(rows.length).fill(null),phaseDirection=Array(rows.length).fill(null),phaseAge=Array(rows.length).fill(null),phaseGainPct=Array(rows.length).fill(null),trendTau=Array(rows.length).fill(null),confirmed=Array(rows.length).fill(false),eligible=Array(rows.length).fill(false),phaseState=Array(rows.length).fill(0),rawEligible=Array(rows.length).fill(false),rawNegativeEligible=Array(rows.length).fill(false),activation=Array(rows.length).fill(false),deactivation=Array(rows.length).fill(false),walk=calculateLatchedWhittakerWalkForward(points,points.length-1,{window,lambda,holdout,trendWindow,positiveConfirmDays,positiveTau,negativeConfirmDays,negativeTau});
  for(let i=Math.max(2,calculationStart);i<points.length;i++){
    const state=walk.states[i];if(!state)continue;signals[i]=state.signal;phaseRegression[i]=state.estimate;phaseDirection[i]=walk.phaseState[i]||state.direction;phaseAge[i]=state.phaseAge;phaseGainPct[i]=state.phaseGainPct;trendTau[i]=state.trendTau;confirmed[i]=state.confirmed;rawEligible[i]=state.positiveEligible;rawNegativeEligible[i]=state.negativeEligible;eligible[i]=walk.regime[i];phaseState[i]=walk.phaseState[i];activation[i]=walk.activation[i];deactivation[i]=walk.deactivation[i];
  }
  return {points,signals,phaseRegression,phaseDirection,phaseAge,phaseGainPct,trendTau,confirmed,eligible,phaseState,rawEligible,rawNegativeEligible,activation,deactivation,window,lambda,holdout,trendWindow,positiveConfirmDays,positiveTau,negativeConfirmDays,negativeTau};
}
function renderWhittakerToolsBacktest(){
  ...
  const range=$("wtBtRange").value||"5y",cutoff=rangeStart(rows.at(-1)[0],range),start=Math.max(1,rows.findIndex(row=>row[0]>=cutoff)),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||1)),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||3)),maxConfirmDays=Math.max(positiveConfirmDays,negativeConfirmDays),calculationStart=Math.max(2,start-window-holdout-maxConfirmDays-5),computed=calculateWhittakerToolsWalkForward(rows,calculationStart),relative=runWhittakerLongOnlyBacktest(rows,computed,start),absolute=runWhittakerFixedNotionalBacktest(rows,computed,start),spread=Math.max(0,+$("wtBtSpread").value||0)/20000,benchmark=buildWhittakerLongOnlyBenchmark(rows,start,relative.capital,spread);
  if($("wtBacktestEntryRule"))$("wtBacktestEntryRule").textContent=`GRÜN: Kendall τ ≥ +${fmt(computed.positiveTau,2)} für ${computed.positiveConfirmDays} Tage → Kauf nächster Open`;
  if($("wtBacktestRegimeRule"))$("wtBacktestRegimeRule").textContent=`ROT: Kendall τ ≤ ${fmt(computed.negativeTau,2)} für ${computed.negativeConfirmDays} Tage → Verkauf nächster Open`;
