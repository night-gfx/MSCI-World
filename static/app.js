...
const WT_BT_IDS = ["wtBtWindow","wtBtExtremeWindow","wtBtPositiveNoise","wtBtNegativeNoise","wtBtCapital","wtBtAbsoluteAmount","wtBtSpread","wtBtRange"];
...
function calculateLatchedWhittakerWalkForward(points,endIndex,{window=250,lambda=1000,holdout=10,extremeWindow=3,positiveNoise=1.5,negativeNoise=1.5}={}){
  const states=Array(points.length).fill(null),endpoints=Array(points.length).fill(null),regime=Array(points.length).fill(false),phaseState=Array(points.length).fill(0),activation=Array(points.length).fill(false),deactivation=Array(points.length).fill(false),rawEligible=Array(points.length).fill(false),rawNegativeEligible=Array(points.length).fill(false);if(points.length)endpoints[0]=+points[0][1];if(points.length>1)endpoints[1]=+points[1][1];let latchedState=0,stateAge=0,positiveStreak=0,negativeStreak=0;const slopeWindow=Math.max(2,Math.floor(extremeWindow)),positiveFactor=Math.max(.1,+positiveNoise||1.5),negativeFactor=Math.max(.1,+negativeNoise||1.5),noiseWindow=20,confirmDays=2;
  for(let i=2;i<=Math.min(endIndex,points.length-1);i++){
    const state=strictWhittakerWalkForwardState(points,i,{window,lambda,holdout,extremeWindow:Math.max(20,slopeWindow)});states[i]=state;
    if(state){
      endpoints[i]=state.whittakerEndpoint;if(i<slopeWindow+noiseWindow){phaseState[i]=latchedState;continue;}const slope=(endpoints[i]/endpoints[i-slopeWindow]-1)*100/slopeWindow,dailyMoves=[];for(let j=i-noiseWindow+1;j<=i;j++)if(endpoints[j-1]>0&&endpoints[j]>0)dailyMoves.push(Math.abs((endpoints[j]/endpoints[j-1]-1)*100));dailyMoves.sort((a,b)=>a-b);const noise=dailyMoves.length?dailyMoves[Math.floor(dailyMoves.length/2)]:0,normalized=noise>1e-9?slope/noise:0;state.riseFromLowPct=normalized;state.dropFromHighPct=normalized;state.positiveEnough=normalized>=positiveFactor;state.negativeEnough=normalized<=-negativeFactor;positiveStreak=state.positiveEnough?positiveStreak+1:0;negativeStreak=state.negativeEnough?negativeStreak+1:0;state.positiveConfirmed=positiveStreak>=confirmDays;state.negativeConfirmed=negativeStreak>=confirmDays;state.positiveEligible=state.positiveConfirmed;state.negativeEligible=state.negativeConfirmed;state.eligible=state.positiveEligible;state.confirmed=state.positiveConfirmed||state.negativeConfirmed;
  ...
  return {states,endpoints,regime,phaseState,activation,deactivation,rawEligible,rawNegativeEligible,outOfSampleStart:0,trainingEnd:null};
}
...
function renderTools(skipEmbeddedBacktest=false){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      const extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||3)),positiveNoise=Math.max(.1,+$("wtBtPositiveNoise").value||1.5),negativeNoise=Math.max(.1,+$("wtBtNegativeNoise").value||1.5),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),walk=calculateLatchedWhittakerWalkForward(analysisPoints,analysisPoints.length-1,{window,lambda,holdout,extremeWindow,positiveNoise,negativeNoise}),currentState=walk.phaseState.at(-1)||0,current=walk.states.at(-1),currentDistance=currentState>0?current?.riseFromLowPct:current?.dropFromHighPct;
      ...
      panels.push({kind:"whittaker-extreme",label:`Normalisierte Whittaker-Steigung (k = ${extremeWindow})`,color:"#0891b2",bar:walk.states.slice(dailyStart).map((state,i)=>{const direction=walk.phaseState[dailyStart+i];return direction>0?state?.riseFromLowPct??null:direction<0?state?.dropFromHighPct??null:null;})});
    }
  }
  ...
  panels.forEach((panel,index)=>{
    ...
    }else if(panel.bar)traces.push({x:dailyX,y:panel.bar,type:"bar",name:panel.label,showlegend:false,marker:{color:panel.bar.map(v=>v>=0?"#16a34a":"#dc2626")},xaxis:`x${axis}`,yaxis:`y${axis}`,hovertemplate:panel.kind==="whittaker-extreme"?"Normierte Steigung: %{y:+.3f}<extra></extra>":"Steigung zum Vortag: %{y:.4f}<extra></extra>"});else panel.series.forEach((series,i)=>traces.push(...splitSigned(dailyX,series,axis,panel.series.length>1?(i?"Kurs - Lower Band":"Kurs - Upper Band"):panel.label)));
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  const points=backtestSignalPoints(rows,"whittaker"),window=Math.max(20,+$("wtBtWindow").value||250),lambda=Math.max(.1,+$("smoothLambda").value||1000),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||3)),positiveNoise=Math.max(.1,+$("wtBtPositiveNoise").value||1.5),negativeNoise=Math.max(.1,+$("wtBtNegativeNoise").value||1.5),signals=Array(rows.length).fill(null),phaseRegression=Array(rows.length).fill(null),phaseDirection=Array(rows.length).fill(null),phaseAge=Array(rows.length).fill(null),phaseGainPct=Array(rows.length).fill(null),riseFromLowPct=Array(rows.length).fill(null),dropFromHighPct=Array(rows.length).fill(null),confirmed=Array(rows.length).fill(false),eligible=Array(rows.length).fill(false),phaseState=Array(rows.length).fill(0),rawEligible=Array(rows.length).fill(false),rawNegativeEligible=Array(rows.length).fill(false),activation=Array(rows.length).fill(false),deactivation=Array(rows.length).fill(false),walk=calculateLatchedWhittakerWalkForward(points,points.length-1,{window,lambda,holdout,extremeWindow,positiveNoise,negativeNoise});
  for(let i=Math.max(2,calculationStart);i<points.length;i++){
...
function renderWhittakerToolsBacktest(){
  ...
  const range=$("wtBtRange").value||"5y",cutoff=rangeStart(rows.at(-1)[0],range),requestedStart=Math.max(1,rows.findIndex(row=>row[0]>=cutoff)),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||3)),calculationStart=2,computed=calculateWhittakerToolsWalkForward(rows,calculationStart),start=Math.max(requestedStart,computed.outOfSampleStart),relative=runWhittakerLongOnlyBacktest(rows,computed,start),absolute=runWhittakerFixedNotionalBacktest(rows,computed,start),spread=Math.max(0,+$("wtBtSpread").value||0)/20000,benchmark=buildWhittakerLongOnlyBenchmark(rows,start,relative.capital,spread);
  if($("wtBacktestEntryRule"))$("wtBacktestEntryRule").textContent=`GRÜN: normierte Whittaker-Steigung ≥ +${fmt(computed.positiveNoise,1)} für 2 Tage → Kauf nächster Open`;
  if($("wtBacktestRegimeRule"))$("wtBacktestRegimeRule").textContent=`ROT: normierte Whittaker-Steigung ≤ −${fmt(computed.negativeNoise,1)} für 2 Tage → Verkauf nächster Open`;
