...
function splitTrend(x,y,name,fillArea=false){
  const segments=[];let current=null,previousSign=null;for(let i=1;i<y.length;i++){if(!Number.isFinite(y[i-1])||!Number.isFinite(y[i])){if(current)segments.push(current);current=null;previousSign=null;continue;}const sign=y[i]>y[i-1]?1:y[i]<y[i-1]?-1:(previousSign||1);if(!current||sign!==previousSign){if(current)segments.push(current);current={sign,x:[x[i-1],x[i]],y:[y[i-1],y[i]]};}else{current.x.push(x[i]);current.y.push(y[i]);}previousSign=sign;}if(current)segments.push(current);return segments.map(segment=>{const rising=segment.sign>0,trace={...lineTrace(segment.x,segment.y,`${name} ${rising?"steigend":"fallend"}`,rising?"#16a34a":"#dc2626","solid",1,2),showlegend:false,hoverinfo:"skip"};if(fillArea){trace.fill="tozeroy";trace.fillcolor=rising?"rgba(22,163,74,.10)":"rgba(220,38,38,.09)";}return trace;});
}
