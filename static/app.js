...
function splitTrend(x,y,name,fillArea=false){
  const segments=[];let current=null,previousSign=null;for(let i=1;i<y.length;i++){if(!Number.isFinite(y[i-1])||!Number.isFinite(y[i])){if(current)segments.push(current);current=null;previousSign=null;continue;}const sign=y[i]>y[i-1]?1:y[i]<y[i-1]?-1:(previousSign||1);if(!current||sign!==previousSign){if(current)segments.push(current);current={sign,x:[x[i-1],x[i]],y:[y[i-1],y[i]]};}else{current.x.push(x[i]);current.y.push(y[i]);}previousSign=sign;}if(current)segments.push(current);return segments.flatMap(segment=>{const rising=segment.sign>0,color=rising?"#16a34a":"#dc2626",line={...lineTrace(segment.x,segment.y,`${name} ${rising?"steigend":"fallend"}`,color,"solid",1,2),showlegend:false,hoverinfo:"skip"};if(!fillArea)return [line];const fill={x:[...segment.x,segment.x.at(-1),segment.x[0]],y:[...segment.y,0,0],type:"scatter",mode:"lines",meta:"trend-fill",showlegend:false,hoverinfo:"skip",fill:"toself",fillcolor:rising?"rgba(22,163,74,.10)":"rgba(220,38,38,.09)",line:{width:0,color:"rgba(0,0,0,0)"}};return [fill,line];});
}
...
function renderTools(){
  ...
  if($("showWhittaker").checked){const fullSmooth=whittakerEilers(dailyFull,+$("smoothLambda").value||1000),smooth=fullSmooth.slice(dailyStart);traces.push(...splitTrend(dailyX,smooth,"Whittaker–Eilers-Smoother · Look-ahead",true));}
