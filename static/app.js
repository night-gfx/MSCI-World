...
const WT_BT_IDS = ["wtBtWindow","wtBtScoreWindow","wtBtPositiveConfirmDays","wtBtPositiveScore","wtBtNegativeConfirmDays","wtBtNegativeScore","wtBtCapital","wtBtAbsoluteAmount","wtBtSpread","wtBtRange"];
...
function volatilityNormalizedTrendScore(points,values,endIndex,lookback=20){
  const length=Math.max(5,Math.floor(lookback)),start=Math.max(0,endIndex-length+1),n=endIndex-start+1;if(n<5)return null;
  const logValues=[];for(let i=start;i<=endIndex;i++){const value=values[i];if(!Number.isFinite(value)||value<=0)return null;logValues.push(Math.log(value));}
  let sx=0,sy=0,sxy=0,sxx=0;for(let i=0;i<n;i++){sx+=i;sy+=logValues[i];sxy+=i*logValues[i];sxx+=i*i;}const denominator=n*sxx-sx*sx;if(Math.abs(denominator)<1e-12)return null;const slope=(n*sxy-sx*sy)/denominator;
  const returns=[];for(let i=start+1;i<=endIndex;i++){const previous=+points[i-1]?.[1],current=+points[i]?.[1];if(previous>0&&current>0)returns.push(Math.log(current/previous));}if(returns.length<2)return null;
  const mean=returns.reduce((sum,value)=>sum+value,0)/returns.length,variance=returns.reduce((sum,value)=>sum+(value-mean)**2,0)/(returns.length-1),volatility=Math.sqrt(variance);if(!Number.isFinite(volatility)||volatility<1e-12)return null;
  return {score:slope*Math.sqrt(n)/volatility,slope,volatility,start,end:endIndex,n};
}
function strictWhittakerWalkForwardState(points,index,{window=250,lambda=1000,holdout=10,scoreWindow=15,positiveScore=.75,negativeScore=-1.25}={}){
  ...
  const trend=volatilityNormalizedTrendScore(sample,smooth,analysisEnd,scoreWindow);if(!trend)return null;const phaseStart=trend.start,phaseModel=linearFitFiltered(sample,smooth,phaseStart,analysisEnd),direction=trend.score>=0?1:-1,phaseAge=trend.n,phaseStartValue=smooth[phaseStart],phaseEndValue=smooth[analysisEnd],phaseGainPct=Number.isFinite(phaseStartValue)&&phaseStartValue!==0?(phaseEndValue/phaseStartValue-1)*100:null;
  const positiveThreshold=Math.max(0,+positiveScore||0),negativeThreshold=Math.min(0,Number.isFinite(+negativeScore)?+negativeScore:0),positiveEnough=trend.score>=positiveThreshold,negativeEnough=trend.score<=negativeThreshold;
  ...
  return {index,first,sample,smooth,analysisEnd,pivots:[],phaseStart,phaseStartGlobal:first+phaseStart,phaseModel,model,fitEnd,fitGlobalEnd:model?first+fitEnd:null,estimate,direction,phaseAge,phaseGainPct,trendScore:trend.score,trendVolatility:trend.volatility,positiveEnough,negativeEnough,signal};
}
function calculateLatchedWhittakerWalkForward(points,endIndex,{window=250,lambda=1000,holdout=10,scoreWindow=15,positiveConfirmDays=3,positiveScore=.75,negativeConfirmDays=3,negativeScore=-1.25}={}){
  const states=Array(points.length).fill(null),regime=Array(points.length).fill(false),phaseState=Array(points.length).fill(0),activation=Array(points.length).fill(false),deactivation=Array(points.length).fill(false),rawEligible=Array(points.length).fill(false),rawNegativeEligible=Array(points.length).fill(false);let latchedState=0;
  let positiveStreak=0,negativeStreak=0;const positiveDays=Math.max(1,Math.floor(positiveConfirmDays)),negativeDays=Math.max(1,Math.floor(negativeConfirmDays));
  for(let i=2;i<=Math.min(endIndex,points.length-1);i++){
    const state=strictWhittakerWalkForwardState(points,i,{window,lambda,holdout,scoreWindow,positiveScore,negativeScore});states[i]=state;
    if(state){
      positiveStreak=state.positiveEnough?positiveStreak+1:0;negativeStreak=state.negativeEnough?negativeStreak+1:0;state.positiveConfirmed=positiveStreak>=positiveDays;state.negativeConfirmed=negativeStreak>=negativeDays;state.positiveEligible=state.positiveConfirmed;state.negativeEligible=state.negativeConfirmed;state.eligible=state.positiveEligible;state.confirmed=state.direction>0?state.positiveConfirmed:state.negativeConfirmed;state.phaseAge=state.direction>0?positiveStreak:negativeStreak;
      rawEligible[i]=state.positiveEligible;rawNegativeEligible[i]=state.negativeEligible;
      if(latchedState<=0&&state.positiveEligible){latchedState=1;activation[i]=true;}
      else if(latchedState>=0&&state.negativeEligible){latchedState=-1;deactivation[i]=true;}
    }
    phaseState[i]=latchedState;regime[i]=latchedState>0;
  }
  return {states,regime,phaseState,activation,deactivation,rawEligible,rawNegativeEligible};
}
...
function installWhittakerSimulationZoomBehavior(){
  ...
  sim.__wtZoomFormatting=event=>{
    ...
    requestAnimationFrame(()=>{try{const start=Array.isArray(event["xaxis.range"])?event["xaxis.range"][0]:event["xaxis.range[0]"],end=Array.isArray(event["xaxis.range"])?event["xaxis.range"][1]:event["xaxis.range[1]"];if(reset)Plotly.relayout(sim,{"yaxis.autorange":true});else if(start!==undefined&&end!==undefined){const t0=new Date(start).getTime(),t1=new Date(end).getTime(),ys=[];for(const trace of sim.data){if(!Array.isArray(trace.x)||!Array.isArray(trace.y))continue;trace.y.forEach((value,i)=>{const tx=new Date(trace.x[i]).getTime(),v=+value;if(Number.isFinite(v)&&Number.isFinite(tx)&&tx>=t0&&tx<=t1)ys.push(v);});}const range=paddedRange([ys]);if(range)Plotly.relayout(sim,{"yaxis.range":range});}}finally{sim.__wtZoomFormattingBusy=false;}},0);
  };
...
function renderWhittakerSimulationGraph(){
  ...
  syncWhittakerSimulationControls(points);const slider=$("whittakerSimSlider"),max=points.length-1,min=+slider.min||2,index=Math.max(min,Math.min(max,+slider.value||max)),snapshot=points.slice(0,index+1),lambda=Math.max(.1,+$("smoothLambda").value||1000),window=Math.max(20,+$("wtBtWindow").value||250),scoreWindow=Math.max(5,Math.floor(+$("wtBtScoreWindow").value||15)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||3)),positiveScore=Math.max(0,Number.isFinite(+$("wtBtPositiveScore").value)?+$("wtBtPositiveScore").value:.75),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||3)),negativeScore=Math.min(0,Number.isFinite(+$("wtBtNegativeScore").value)?+$("wtBtNegativeScore").value:-1.25),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),smooth=whittakerEilers(snapshot,lambda),asOf=snapshot.at(-1)[0],rangeCutoff=toolRange==="max"?-Infinity:rangeStart(asOf,toolRange),displayStart=Math.max(0,snapshot.findIndex(point=>point[0]>=rangeCutoff)),x=pointDates(snapshot.slice(displayStart)),price=pointPrices(snapshot.slice(displayStart)),smoothVisible=smooth.slice(displayStart),walk=calculateLatchedWhittakerWalkForward(points,index,{window,lambda,holdout,scoreWindow,positiveConfirmDays,positiveScore,negativeConfirmDays,negativeScore}),phaseState=walk.phaseState,states=walk.states,activation=walk.activation,deactivation=walk.deactivation;
  const phaseAge=states.map(state=>state?.phaseAge??null),trendScores=states.map(state=>state?.trendScore??null),stateVisible=phaseState.slice(displayStart,index+1),neutral=smoothVisible.map((v,i)=>stateVisible[i]===0?v:null),green=smoothVisible.map((v,i)=>stateVisible[i]>0?v:null),red=smoothVisible.map((v,i)=>stateVisible[i]<0?v:null),custom=x.map((_,i)=>[phaseAge[displayStart+i],trendScores[displayStart+i]]),traces=[{...lineTrace(x,price,"Tatsächlicher Kurs","#0f172a","solid",1,1.7),hovertemplate:"Kurs: %{y:.4f}<extra></extra>"},{...lineTrace(x,neutral,"Neutral","#94a3b8","solid",1,2.6),connectgaps:false,customdata:custom,hovertemplate:"Neutral<br>Whittaker: %{y:.4f}<br>Bestätigungsserie: %{customdata[0]} Tage<br>Trend-Score: %{customdata[1]:+.2f}<extra></extra>"},{...lineTrace(x,green,"Positive Phase · GRÜN","#16a34a","solid",1,3),connectgaps:false,customdata:custom,hovertemplate:"GRÜN<br>Whittaker: %{y:.4f}<br>Bestätigungsserie: %{customdata[0]} Tage<br>Trend-Score: %{customdata[1]:+.2f}<extra></extra>"},{...lineTrace(x,red,"Negative Phase · ROT","#dc2626","solid",1,3),connectgaps:false,customdata:custom,hovertemplate:"ROT<br>Whittaker: %{y:.4f}<br>Bestätigungsserie: %{customdata[0]} Tage<br>Trend-Score: %{customdata[1]:+.2f}<extra></extra>"}];
  const addSwitchMarkers=(flags,name,color,symbol)=>{const indexes=flags.map((value,i)=>value&&i>=displayStart&&i<=index?i:-1).filter(i=>i>=0);if(indexes.length)traces.push({x:indexes.map(i=>new Date(points[i][0])),y:indexes.map(i=>states[i]?.smooth?.at(-1)??points[i][1]),type:"scatter",mode:"markers",name,marker:{symbol,size:10,color,line:{color:"#fff",width:1}},customdata:indexes.map(i=>[phaseAge[i],trendScores[i]]),hovertemplate:`${name}<br>Bestätigungsserie: %{customdata[0]} Tage<br>Trend-Score: %{customdata[1]:+.2f}<extra></extra>`});};
...
function renderTools(skipEmbeddedBacktest=false){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      const scoreWindow=Math.max(5,Math.floor(+$("wtBtScoreWindow").value||15)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||3)),positiveScore=Math.max(0,Number.isFinite(+$("wtBtPositiveScore").value)?+$("wtBtPositiveScore").value:.75),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||3)),negativeScore=Math.min(0,Number.isFinite(+$("wtBtNegativeScore").value)?+$("wtBtNegativeScore").value:-1.25),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),walk=calculateLatchedWhittakerWalkForward(analysisPoints,analysisPoints.length-1,{window,lambda,holdout,scoreWindow,positiveConfirmDays,positiveScore,negativeConfirmDays,negativeScore}),currentState=walk.phaseState.at(-1)||0,currentScore=walk.states.at(-1)?.trendScore;
      if($("wtCurrentPhase")){const label=currentState>0?"Positiv":currentState<0?"Negativ":"Neutral";$("wtCurrentPhase").textContent=`${label}${Number.isFinite(currentScore)?` · Score ${fmt(currentScore,2)}`:""}`;$("wtCurrentPhase").className=currentState>0?"phase-positive":currentState<0?"phase-negative":"";}
      panels.push({kind:"whittaker-trend-score",label:`Volatilitätsnormalisierter Trend-Score (${scoreWindow} Tage)`,color:"#0891b2",bar:walk.states.slice(dailyStart).map(state=>state?.trendScore??null)});
    }
  }
  ...
  panels.forEach((panel,index)=>{
    ...
    }else if(panel.bar)traces.push({x:dailyX,y:panel.bar,type:"bar",name:panel.label,showlegend:false,marker:{color:panel.bar.map(v=>v>=0?"#16a34a":"#dc2626")},xaxis:`x${axis}`,yaxis:`y${axis}`,hovertemplate:panel.kind==="whittaker-trend-score"?"Trend-Score: %{y:+.3f}<extra></extra>":"Steigung zum Vortag: %{y:.4f}<extra></extra>"});else panel.series.forEach((series,i)=>traces.push(...splitSigned(dailyX,series,axis,panel.series.length>1?(i?"Kurs - Lower Band":"Kurs - Upper Band"):panel.label)));
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  const points=backtestSignalPoints(rows,"whittaker"),window=Math.max(20,+$("wtBtWindow").value||250),lambda=Math.max(.1,+$("smoothLambda").value||1000),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),scoreWindow=Math.max(5,Math.floor(+$("wtBtScoreWindow").value||15)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||3)),positiveScore=Math.max(0,Number.isFinite(+$("wtBtPositiveScore").value)?+$("wtBtPositiveScore").value:.75),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||3)),negativeScore=Math.min(0,Number.isFinite(+$("wtBtNegativeScore").value)?+$("wtBtNegativeScore").value:-1.25),signals=Array(rows.length).fill(null),phaseRegression=Array(rows.length).fill(null),phaseDirection=Array(rows.length).fill(null),phaseAge=Array(rows.length).fill(null),phaseGainPct=Array(rows.length).fill(null),trendScore=Array(rows.length).fill(null),confirmed=Array(rows.length).fill(false),eligible=Array(rows.length).fill(false),phaseState=Array(rows.length).fill(0),rawEligible=Array(rows.length).fill(false),rawNegativeEligible=Array(rows.length).fill(false),activation=Array(rows.length).fill(false),deactivation=Array(rows.length).fill(false),walk=calculateLatchedWhittakerWalkForward(points,points.length-1,{window,lambda,holdout,scoreWindow,positiveConfirmDays,positiveScore,negativeConfirmDays,negativeScore});
  for(let i=Math.max(2,calculationStart);i<points.length;i++){
    const state=walk.states[i];if(!state)continue;signals[i]=state.signal;phaseRegression[i]=state.estimate;phaseDirection[i]=walk.phaseState[i]||state.direction;phaseAge[i]=state.phaseAge;phaseGainPct[i]=state.phaseGainPct;trendScore[i]=state.trendScore;confirmed[i]=state.confirmed;rawEligible[i]=state.positiveEligible;rawNegativeEligible[i]=state.negativeEligible;eligible[i]=walk.regime[i];phaseState[i]=walk.phaseState[i];activation[i]=walk.activation[i];deactivation[i]=walk.deactivation[i];
  }
  return {points,signals,phaseRegression,phaseDirection,phaseAge,phaseGainPct,trendScore,confirmed,eligible,phaseState,rawEligible,rawNegativeEligible,activation,deactivation,window,lambda,holdout,scoreWindow,positiveConfirmDays,positiveScore,negativeConfirmDays,negativeScore};
}
function renderWhittakerToolsBacktest(){
  ...
  const range=$("wtBtRange").value||"5y",cutoff=rangeStart(rows.at(-1)[0],range),start=Math.max(1,rows.findIndex(row=>row[0]>=cutoff)),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),positiveConfirmDays=Math.max(1,Math.floor(+$("wtBtPositiveConfirmDays").value||3)),negativeConfirmDays=Math.max(1,Math.floor(+$("wtBtNegativeConfirmDays").value||3)),maxConfirmDays=Math.max(positiveConfirmDays,negativeConfirmDays),calculationStart=Math.max(2,start-window-holdout-maxConfirmDays-5),computed=calculateWhittakerToolsWalkForward(rows,calculationStart),relative=runWhittakerLongOnlyBacktest(rows,computed,start),absolute=runWhittakerFixedNotionalBacktest(rows,computed,start),spread=Math.max(0,+$("wtBtSpread").value||0)/20000,benchmark=buildWhittakerLongOnlyBenchmark(rows,start,relative.capital,spread);
  if($("wtBacktestEntryRule"))$("wtBacktestEntryRule").textContent=`GRÜN: Score ≥ +${fmt(computed.positiveScore,1)} für ${computed.positiveConfirmDays} Tage → Kauf nächster Open`;
  if($("wtBacktestRegimeRule"))$("wtBacktestRegimeRule").textContent=`ROT: Score ≤ ${fmt(computed.negativeScore,1)} für ${computed.negativeConfirmDays} Tage → Verkauf nächster Open`;
