...
function strictWhittakerWalkForwardState(points,index,{window=250,lambda=1000,holdout=10,confirmDays=5,minPositivePct=1}={}){
  ...
  const phaseModel=linearFitFiltered(sample,smooth,phaseStart,analysisEnd),direction=phaseModel?.slope>=0?1:-1,phaseAge=analysisEnd-phaseStart+1,phaseStartValue=smooth[phaseStart],phaseEndValue=smooth[analysisEnd],phaseGainPct=Number.isFinite(phaseStartValue)&&phaseStartValue!==0?(phaseEndValue/phaseStartValue-1)*100:null,confirmed=phaseAge>=Math.max(1,Math.floor(confirmDays)),threshold=Math.max(0,+minPositivePct||0),positiveEnough=direction>0&&Number.isFinite(phaseGainPct)&&phaseGainPct>=threshold,negativeEnough=direction<0&&Number.isFinite(phaseGainPct)&&phaseGainPct<=-threshold,positiveEligible=confirmed&&positiveEnough,negativeEligible=confirmed&&negativeEnough,eligible=positiveEligible;
  ...
  return {index,first,sample,smooth,analysisEnd,pivots,phaseStart,phaseStartGlobal:first+phaseStart,phaseModel,model,fitEnd,fitGlobalEnd:model?first+fitEnd:null,estimate,direction,phaseAge,phaseGainPct,confirmed,positiveEnough,negativeEnough,positiveEligible,negativeEligible,eligible,signal};
}
function calculateLatchedWhittakerWalkForward(points,endIndex,{window=250,lambda=1000,holdout=10,confirmDays=5,minPositivePct=1}={}){
  ...
  for(let i=2;i<=Math.min(endIndex,points.length-1);i++){
    const state=strictWhittakerWalkForwardState(points,i,{window,lambda,holdout,confirmDays,minPositivePct});states[i]=state;
    if(state){
...
function renderWhittakerSimulationGraph(){
  ...
  syncWhittakerSimulationControls(points);const slider=$("whittakerSimSlider"),max=points.length-1,min=+slider.min||2,index=Math.max(min,Math.min(max,+slider.value||max)),snapshot=points.slice(0,index+1),lambda=Math.max(.1,+$("smoothLambda").value||1000),window=Math.max(20,+$("wtBtWindow").value||250),confirmDays=Math.max(1,Math.floor(+$("wtBtConfirmDays").value||5)),minPositivePct=Math.max(0,+$("wtBtMinPositivePct").value||0),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),smooth=whittakerEilers(snapshot,lambda),asOf=snapshot.at(-1)[0],rangeCutoff=toolRange==="max"?-Infinity:rangeStart(asOf,toolRange),displayStart=Math.max(0,snapshot.findIndex(point=>point[0]>=rangeCutoff)),x=pointDates(snapshot.slice(displayStart)),price=pointPrices(snapshot.slice(displayStart)),smoothVisible=smooth.slice(displayStart),walk=calculateLatchedWhittakerWalkForward(points,index,{window,lambda,holdout,confirmDays,minPositivePct}),phaseState=walk.phaseState,states=walk.states,activation=walk.activation,deactivation=walk.deactivation;
  if($("wtPositiveRule"))$("wtPositiveRule").textContent=`mind. ${confirmDays} Handelstage und Phasenanstieg ≥ +${fmt(minPositivePct,1)} % → Zustand GRÜN`;
  if($("wtNegativeRule"))$("wtNegativeRule").textContent=`mind. ${confirmDays} Handelstage und Phasenrückgang ≤ −${fmt(minPositivePct,1)} % → Zustand ROT; Long wird am nächsten Open verkauft`;
  if($("wtStateRule"))$("wtStateRule").textContent="Neutral nur bis zur ersten bestätigten Phase. Danach bleibt jeder damalige Zustand unverändert und wechselt ausschließlich bei einer bestätigten Gegenphase zwischen GRÜN und ROT.";
  const phaseAge=states.map(state=>state?.phaseAge??null),phaseGain=states.map(state=>state?.phaseGainPct??null),stateVisible=phaseState.slice(displayStart,index+1),neutral=smoothVisible.map((v,i)=>stateVisible[i]===0?v:null),green=smoothVisible.map((v,i)=>stateVisible[i]>0?v:null),red=smoothVisible.map((v,i)=>stateVisible[i]<0?v:null),walkRegression=states.map(state=>state?.estimate??null),regVisible=walkRegression.slice(displayStart,index+1),neutralRegression=regVisible.map((v,i)=>stateVisible[i]===0?v:null),greenRegression=regVisible.map((v,i)=>stateVisible[i]>0?v:null),redRegression=regVisible.map((v,i)=>stateVisible[i]<0?v:null),custom=x.map((_,i)=>[phaseAge[displayStart+i],phaseGain[displayStart+i]]),traces=[{...lineTrace(x,price,"Tatsächlicher Kurs","#0f172a","solid",1,1.7),hovertemplate:"Kurs: %{y:.4f}<extra></extra>"},{...lineTrace(x,neutral,"Neutral · noch keine bestätigte Phase","#94a3b8","solid",1,2.6),connectgaps:false,customdata:custom,hovertemplate:"Neutral<br>Whittaker: %{y:.4f}<br>Phasenalter: %{customdata[0]} Tage<br>Phasenbewegung: %{customdata[1]:+.2f} %<extra></extra>"},{...lineTrace(x,green,"Bestätigte positive Phase · GRÜN","#16a34a","solid",1,3),connectgaps:false,customdata:custom,hovertemplate:"GRÜN · historisch fixiert<br>Whittaker: %{y:.4f}<br>Phasenalter: %{customdata[0]} Tage<br>Phasenbewegung: %{customdata[1]:+.2f} %<extra></extra>"},{...lineTrace(x,red,"Bestätigte negative Phase · ROT","#dc2626","solid",1,3),connectgaps:false,customdata:custom,hovertemplate:"ROT · historisch fixiert<br>Whittaker: %{y:.4f}<br>Phasenalter: %{customdata[0]} Tage<br>Phasenbewegung: %{customdata[1]:+.2f} %<extra></extra>"},{...lineTrace(x,neutralRegression,"Walk-forward-Regression · neutral","#cbd5e1","solid",1,1.2),showlegend:false,connectgaps:false,hoverinfo:"skip"},{...lineTrace(x,greenRegression,"Walk-forward-Regression · grün","#15803d","solid",1,1.8),showlegend:false,connectgaps:false,hovertemplate:"Regression: %{y:.4f}<extra></extra>"},{...lineTrace(x,redRegression,"Walk-forward-Regression · rot","#b91c1c","solid",1,1.8),showlegend:false,connectgaps:false,hovertemplate:"Regression: %{y:.4f}<extra></extra>"}];
  ...
  addSwitchMarkers(activation,"Wechsel auf GRÜN","#16a34a","triangle-up");addSwitchMarkers(deactivation,"Wechsel auf ROT / OUT","#dc2626","triangle-down");
  const totalHeight=Math.max(520,mainChart?.clientHeight||parseInt(mainChart?.style?.height)||560);panel.style.height=`${totalHeight}px`;panel.style.minHeight=`${totalHeight}px`;const controls=panel.querySelector(".whittaker-simulation-topbar"),controlsHeight=controls?.offsetHeight||220,chartHeight=Math.max(280,totalHeight-controlsHeight-8),xRange=x.length?[new Date(x[0]),new Date(x.at(-1))]:undefined,layout={...baseLayout(),paper_bgcolor:"#fff",plot_bgcolor:"#fff",height:chartHeight,dragmode:"zoom",uirevision:`wt-sim-${toolRange}`,showlegend:true,margin:{l:48,r:18,t:44,b:42},legend:{orientation:"h",x:0,y:1.055,font:{size:9},bgcolor:"rgba(255,255,255,0)"},xaxis:{...axisBase(snapshot.slice(displayStart)),type:"date",range:xRange,hoverformat:"%d.%m.%Y",showgrid:false,showline:false,ticks:"",tickfont:{size:10,color:"#64748b"},automargin:true},yaxis:{range:paddedRange([price,smoothVisible,neutralRegression,greenRegression,redRegression]),showgrid:false,showline:false,zeroline:false,tickformat:".3f",tickfont:{size:10,color:"#64748b"},automargin:true},annotations:[{xref:"paper",yref:"paper",x:0,y:1.015,text:`<b>Informationsstand ${fmtDate(asOf)}</b>`,showarrow:false,xanchor:"left",font:{size:10,color:"#475569"}}]};
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  ...
  return {points,signals,phaseRegression,phaseDirection,phaseAge,phaseGainPct,confirmed,eligible,phaseState,rawEligible,rawNegativeEligible,activation,deactivation,window,lambda,holdout,confirmDays,minPositivePct};
}
