...
function calculateLatchedWhittakerWalkForward(points,endIndex,{window=250,lambda=1000,holdout=10,extremeWindow=40,extremePct=2}={}){
  ...
  let stateAge=0,lastSwitchIndex=0;const lookback=Math.max(2,Math.floor(extremeWindow)),threshold=Math.max(0,+extremePct||0);
  for(let i=2;i<=Math.min(endIndex,points.length-1);i++){
    const state=strictWhittakerWalkForwardState(points,i,{window,lambda,holdout,extremeWindow:lookback});states[i]=state;
    if(state){
      endpoints[i]=state.whittakerEndpoint;const start=Math.max(0,i-lookback+1,lastSwitchIndex),visible=endpoints.slice(start,i+1).filter(Number.isFinite);if(visible.length<2){phaseState[i]=latchedState;regime[i]=latchedState>0;continue;}const low=Math.min(...visible),high=Math.max(...visible),current=state.whittakerEndpoint,riseFromLowPct=low!==0?(current/low-1)*100:null,dropFromHighPct=high!==0?(current/high-1)*100:null;state.riseFromLowPct=riseFromLowPct;state.dropFromHighPct=dropFromHighPct;state.positiveEnough=Number.isFinite(riseFromLowPct)&&riseFromLowPct>=threshold;state.negativeEnough=Number.isFinite(dropFromHighPct)&&dropFromHighPct<=-threshold;state.positiveConfirmed=state.positiveEnough;state.negativeConfirmed=state.negativeEnough;state.positiveEligible=state.positiveEnough;state.negativeEligible=state.negativeEnough;state.eligible=state.positiveEligible;state.confirmed=state.positiveEnough||state.negativeEnough;
      rawEligible[i]=state.positiveEligible;rawNegativeEligible[i]=state.negativeEligible;
      const previousState=latchedState;if(latchedState<0&&state.positiveEligible){latchedState=1;activation[i]=true;}else if(latchedState>0&&state.negativeEligible){latchedState=-1;deactivation[i]=true;}else if(latchedState===0&&(state.positiveEligible||state.negativeEligible)){if(state.positiveEligible&&!state.negativeEligible||state.positiveEligible&&riseFromLowPct>=Math.abs(dropFromHighPct)){latchedState=1;activation[i]=true;}else{latchedState=-1;deactivation[i]=true;}}if(latchedState!==previousState)lastSwitchIndex=i;stateAge=latchedState===previousState?stateAge+1:1;state.phaseAge=stateAge;state.direction=latchedState||state.direction;
    }
...
function renderTools(skipEmbeddedBacktest=false){
  ...
  if($("showWhittaker").checked){
    ...
    if(analysisPoints.length>=3){
      ...
      const extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||40)),extremePct=Math.max(.1,+$("wtBtExtremePct").value||2),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),walk=calculateLatchedWhittakerWalkForward(analysisPoints,analysisPoints.length-1,{window,lambda,holdout,extremeWindow,extremePct}),currentState=walk.phaseState.at(-1)||0,current=walk.states.at(-1),currentDistance=currentState>0?current?.riseFromLowPct:current?.dropFromHighPct;
      ...
      panels.push({kind:"whittaker-extreme",label:`Whittaker-Abstand zum relevanten Extrem (${extremeWindow} Tage)`,color:"#0891b2",bar:walk.states.slice(dailyStart).map((state,i)=>{const direction=walk.phaseState[dailyStart+i];return direction>0?state?.riseFromLowPct??null:direction<0?state?.dropFromHighPct??null:null;})});
    }
  }
  ...
  panels.forEach((panel,index)=>{
    ...
    }else if(panel.bar)traces.push({x:dailyX,y:panel.bar,type:"bar",name:panel.label,showlegend:false,marker:{color:panel.bar.map(v=>v>=0?"#16a34a":"#dc2626")},xaxis:`x${axis}`,yaxis:`y${axis}`,hovertemplate:panel.kind==="whittaker-extreme"?"Abstand zum Extrem: %{y:+.3f} %<extra></extra>":"Steigung zum Vortag: %{y:.4f}<extra></extra>"});else panel.series.forEach((series,i)=>traces.push(...splitSigned(dailyX,series,axis,panel.series.length>1?(i?"Kurs - Lower Band":"Kurs - Upper Band"):panel.label)));
...
function calculateWhittakerToolsWalkForward(rows,calculationStart){
  const points=backtestSignalPoints(rows,"whittaker"),window=Math.max(20,+$("wtBtWindow").value||250),lambda=Math.max(.1,+$("smoothLambda").value||1000),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||40)),extremePct=Math.max(.1,+$("wtBtExtremePct").value||2),signals=Array(rows.length).fill(null),phaseRegression=Array(rows.length).fill(null),phaseDirection=Array(rows.length).fill(null),phaseAge=Array(rows.length).fill(null),phaseGainPct=Array(rows.length).fill(null),riseFromLowPct=Array(rows.length).fill(null),dropFromHighPct=Array(rows.length).fill(null),confirmed=Array(rows.length).fill(false),eligible=Array(rows.length).fill(false),phaseState=Array(rows.length).fill(0),rawEligible=Array(rows.length).fill(false),rawNegativeEligible=Array(rows.length).fill(false),activation=Array(rows.length).fill(false),deactivation=Array(rows.length).fill(false),walk=calculateLatchedWhittakerWalkForward(points,points.length-1,{window,lambda,holdout,extremeWindow,extremePct});
  for(let i=Math.max(2,calculationStart);i<points.length;i++){
  ...
  return {points,signals,phaseRegression,phaseDirection,phaseAge,phaseGainPct,riseFromLowPct,dropFromHighPct,confirmed,eligible,phaseState,rawEligible,rawNegativeEligible,activation,deactivation,window,lambda,holdout,extremeWindow,extremePct};
}
function renderWhittakerToolsBacktest(){
  ...
  const range=$("wtBtRange").value||"5y",cutoff=rangeStart(rows.at(-1)[0],range),start=Math.max(1,rows.findIndex(row=>row[0]>=cutoff)),window=Math.max(20,+$("wtBtWindow").value||250),holdout=Math.max(0,Math.floor(+$("whittakerRegressionHoldout").value||0)),extremeWindow=Math.max(2,Math.floor(+$("wtBtExtremeWindow").value||40)),calculationStart=Math.max(2,start-window-holdout-extremeWindow-5),computed=calculateWhittakerToolsWalkForward(rows,calculationStart),relative=runWhittakerLongOnlyBacktest(rows,computed,start),absolute=runWhittakerFixedNotionalBacktest(rows,computed,start),spread=Math.max(0,+$("wtBtSpread").value||0)/20000,benchmark=buildWhittakerLongOnlyBenchmark(rows,start,relative.capital,spread);
  if($("wtBacktestEntryRule"))$("wtBacktestEntryRule").textContent=`GRÜN: Whittaker ≥ +${fmt(computed.extremePct,1)} % über ${computed.extremeWindow}-Tage-Tief → Kauf nächster Open`;
  if($("wtBacktestRegimeRule"))$("wtBacktestRegimeRule").textContent=`ROT: Whittaker ≤ −${fmt(computed.extremePct,1)} % unter ${computed.extremeWindow}-Tage-Hoch → Verkauf nächster Open`;
  const extremeRows=rows.slice(start),extremeDates=extremeRows.map(row=>new Date(row[0])),extremeDistance=extremeRows.map((_,offset)=>{const i=start+offset,state=computed.phaseState[i];return state>0?computed.riseFromLowPct[i]:state<0?computed.dropFromHighPct[i]:null;}),extremeTraces=[{x:extremeDates,y:extremeDistance,type:"bar",name:"Abstand zum Extrem",showlegend:false,marker:{color:extremeDistance.map(value=>Number.isFinite(value)&&value>=0?"#16a34a":"#dc2626")},hovertemplate:"Abstand zum Extrem: %{y:+.2f} %<extra></extra>"}],extremeLayout={...baseLayout(),height:260,showlegend:false,bargap:.04,margin:{l:52,r:18,t:12,b:42},xaxis:{...axisBase(extremeRows.map(row=>[row[0],row[4]])),hoverformat:"%d.%m.%Y"},yaxis:{range:paddedRange([extremeDistance,[computed.extremePct,-computed.extremePct]],true),ticksuffix:" %",tickformat:".1f",showgrid:true,gridcolor:"#f8fafc",zeroline:true,zerolinecolor:"#64748b"},shapes:[{type:"line",xref:"paper",x0:0,x1:1,yref:"y",y0:computed.extremePct,y1:computed.extremePct,line:{color:"#16a34a",width:1,dash:"dot"}},{type:"line",xref:"paper",x0:0,x1:1,yref:"y",y0:-computed.extremePct,y1:-computed.extremePct,line:{color:"#dc2626",width:1,dash:"dot"}}]};
  for(let i=start;i<rows.length;i++){if(!computed.activation[i]&&!computed.deactivation[i])continue;extremeLayout.shapes.push({type:"line",xref:"x",x0:new Date(rows[i][0]),x1:new Date(rows[i][0]),yref:"paper",y0:0,y1:1,line:{color:computed.activation[i]?"rgba(22,163,74,.65)":"rgba(220,38,38,.65)",width:1,dash:"dash"}});}
  Plotly.react("wtExtremeDistanceChart",extremeTraces,extremeLayout,PLOT_CONFIG).then(()=>installVisibleYAutoscale("wtExtremeDistanceChart"));
...
document.querySelectorAll(".tab").forEach(button=>button.onclick=()=>{activeTab=button.dataset.tab;document.querySelectorAll(".tab").forEach(b=>b.classList.toggle("active",b===button));$("toolsTab").classList.toggle("hidden",activeTab!=="tools");$("analyticsTab").classList.toggle("hidden",activeTab!=="analytics");setTimeout(()=>{if(activeTab==="tools"){Plotly.Plots.resize("toolsChart");for(const id of ["whittakerSimulationChart","wtExtremeDistanceChart","wtRelativeChart","wtRelativeMonthlyChart","wtAbsoluteMonthlyChart"]){const graph=plotlyGraph(id);if(graph)Plotly.Plots.resize(graph);}}else Plotly.Plots.resize("analyticsChart");},0);});
