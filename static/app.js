...
function whittakerEilers(points,lambdaValue){
  const y=pointPrices(points),n=y.length;if(n<3)return y;const lambda=Math.max(.1,+lambdaValue||1000),m=n-1,dot=(a,b)=>a.reduce((sum,value,index)=>sum+value*b[index],0),applyFull=(values,identity=true)=>{const out=identity?[...values]:Array(n).fill(0);for(let index=0;index<n-2;index++){const difference=values[index]-2*values[index+1]+values[index+2],weighted=lambda*difference;out[index]+=weighted;out[index+1]-=2*weighted;out[index+2]+=weighted;}return out;},applyUnknown=values=>applyFull([...values,0]).slice(0,m),fixed=Array(n).fill(0);fixed[n-1]=y[n-1];const fixedEffect=applyFull(fixed,false),b=y.slice(0,m).map((value,index)=>value-fixedEffect[index]),diagonal=Array(n).fill(1);for(let index=0;index<n-2;index++){diagonal[index]+=lambda;diagonal[index+1]+=4*lambda;diagonal[index+2]+=lambda;}let x=y.slice(0,m),product=applyUnknown(x),r=b.map((value,index)=>value-product[index]),z=r.map((value,index)=>value/diagonal[index]),direction=[...z],rz=dot(r,z),scale=Math.max(1,Math.sqrt(dot(b,b)));
  for(let iteration=0;iteration<500&&Math.sqrt(dot(r,r))>scale*1e-9;iteration++){product=applyUnknown(direction);const denominator=dot(direction,product);if(Math.abs(denominator)<1e-20)break;const alpha=rz/denominator;x=x.map((value,index)=>value+alpha*direction[index]);r=r.map((value,index)=>value-alpha*product[index]);z=r.map((value,index)=>value/diagonal[index]);const nextRz=dot(r,z),beta=nextRz/rz;direction=z.map((value,index)=>value+beta*direction[index]);rz=nextRz;}
  return [...x,y[n-1]];
}
...
function splitTrend(x,y,name){
  ...
  return [
    {...lineTrace(risingX,risingY,`${name} steigend`,"#16a34a","solid",1,2),showlegend:false,hoverinfo:"skip",fill:"tozeroy",fillcolor:"rgba(22,163,74,.10)"},
    {...lineTrace(fallingX,fallingY,`${name} fallend`,"#dc2626","solid",1,2),showlegend:false,hoverinfo:"skip",fill:"tozeroy",fillcolor:"rgba(220,38,38,.09)"}
  ];
}
...
function backtestSignals(rows,type){const points=rows.map(row=>[row[0],row[4]]),signals=Array(rows.length).fill(null);
  if(type==="kalman"){const values=kalman2d(points,Math.max(.001,+$("btKalmanQ").value||1),Math.max(.001,+$("btKalmanR").value||25));for(let i=1;i<values.length;i++)signals[i]=filterSlope(values,i);return signals;}
  if(type==="regression"){const window=Math.max(3,+$("btRegressionWindow").value||182);for(let i=2;i<points.length;i++){const fitted=regression(points.slice(0,i+1),window);signals[i]=filterSlope(fitted,fitted.length-1);}return signals;}
  const window=Math.max(10,+$("btWhittakerWindow").value||250),lambda=Math.max(.1,+$("btWhittakerLambda").value||1000);for(let i=2;i<points.length;i++){const sample=points.slice(Math.max(0,i-window+1),i+1),smooth=whittakerEilers(sample,lambda);signals[i]=filterSlope(smooth,smooth.length-1);}return signals;
}
function runBacktest(rows,signals,type){const capital=Math.max(10,+$("btCapital").value||10000),spread=Math.max(0,+$("btSpread").value||0)/20000,buyThreshold=+$(type==="regression"?"btRegressionBuy":type==="kalman"?"btKalmanBuy":"btWhittakerBuy").value||0,sellThreshold=+$(type==="regression"?"btRegressionSell":type==="kalman"?"btKalmanSell":"btWhittakerSell").value||0,cutoff=rangeStart(rows.at(-1)[0],backtestRange),start=Math.max(1,rows.findIndex(row=>row[0]>=cutoff));let cash=capital,shares=0,basis=0,taxes=0,fees=0,peak=capital,maxDrawdown=0;const curve=[],trades=[];
  for(let i=start;i<rows.length;i++){const signal=signals[i-1],open=rows[i][1],close=rows[i][4];if(Number.isFinite(signal)&&!shares&&signal>buyThreshold&&cash>1){const ask=open*(1+spread),spend=cash-1;shares=spend/ask;basis=cash;cash=0;fees+=1;trades.push({side:"Kauf",signalDate:rows[i-1][0],date:rows[i][0],price:ask,cost:1,tax:0,capital:shares*close});}else if(Number.isFinite(signal)&&shares&&signal<sellThreshold){const bid=open*(1-spread),gross=shares*bid-1,profit=gross-basis,tax=Math.max(0,profit)*.25;cash=gross-tax;taxes+=tax;fees+=1;shares=0;basis=0;trades.push({side:"Verkauf",signalDate:rows[i-1][0],date:rows[i][0],price:bid,cost:1,tax,capital:cash});}const equity=cash+shares*close;peak=Math.max(peak,equity);maxDrawdown=Math.min(maxDrawdown,equity/peak-1);curve.push([rows[i][0],equity]);}
...
function renderBacktestTrades(){const result=backtestResults[$("backtestTradeStrategy").value],body=$("backtestTradeRows");body.innerHTML=result?.trades.length?result.trades.map(trade=>`<tr><td>${trade.side}</td><td>${fmtDate(trade.signalDate)}</td><td>${fmtDate(trade.date)}</td><td>${fmt(trade.price,4)} €</td><td>${fmt(trade.cost)} €</td><td>${fmt(trade.tax)} €</td><td>${fmt(trade.capital)} €</td></tr>`).join(""):`<tr><td colspan="7">Keine Trades für diese Regel.</td></tr>`;}
function renderBacktest(){if(!payload)return;const rows=backtestOhlc();if(rows.length<3){$("backtestChart").innerHTML="<div class='backtest-empty'>Für dieses Instrument fehlen noch Tages-OHLC-Daten.</div>";return;}const definitions=[{type:"regression",on:$("btRegressionOn").checked,color:"#7c3aed"},{type:"kalman",on:$("btKalmanOn").checked,color:"#db2777"},{type:"whittaker",on:$("btWhittakerOn").checked,color:"#0891b2"}],enabled=definitions.filter(item=>item.on);backtestResults={};for(const item of enabled)backtestResults[item.type]=runBacktest(rows,backtestSignals(rows,item.type),item.type);const traces=enabled.map(item=>{const result=backtestResults[item.type];return {...lineTrace(pointDates(result.curve),pointPrices(result.curve),backtestName(item.type),item.color,"solid",1,2.2),hovertemplate:`${backtestName(item.type)}: %{y:.2f} €<extra></extra>`};});const cutoff=rangeStart(rows.at(-1)[0],backtestRange),visible=rows.filter(row=>row[0]>=cutoff),initial=Math.max(10,+$("btCapital").value||10000),spread=Math.max(0,+$("btSpread").value||0)/20000,firstAsk=visible[0][1]*(1+spread),buyShares=(initial-1)/firstAsk,buyHold=visible.map(row=>[row[0],buyShares*row[4]]);traces.unshift({...lineTrace(pointDates(buyHold),pointPrices(buyHold),"Buy & Hold (brutto, offen)","#0f172a","dot",1,1.7),hovertemplate:"Buy & Hold: %{y:.2f} €<extra></extra>"});const allValues=traces.map(trace=>trace.y),layout={...baseLayout(),height:560,showlegend:true,margin:{l:62,r:24,t:50,b:42},legend:{orientation:"h",x:0,y:1.08},xaxis:{...axisBase(visible.map(row=>[row[0],row[4]])),hoverformat:"%d.%m.%Y"},yaxis:{range:paddedRange(allValues),tickformat:",.0f",ticksuffix:" €",showgrid:true,gridcolor:"#f1f5f9"}};Plotly.react("backtestChart",traces,layout,PLOT_CONFIG).then(()=>installVisibleYAutoscale("backtestChart"));$("backtestMetrics").innerHTML=enabled.map(item=>{const result=backtestResults[item.type];return `<div class="card backtest-metric-card"><strong>${backtestName(item.type)}</strong><div><span>Endkapital</span><b>${fmt(result.final)} €</b></div><div><span>Rendite</span><b class="${result.returnPct>=0?"positive":"negative"}">${pct(result.returnPct)}</b></div><div><span>Max. Drawdown</span><b>${pct(result.maxDrawdown)}</b></div><div><span>Ausführungen</span><b>${result.trades.length}</b></div><div><span>Gebühren</span><b>${fmt(result.fees)} €</b></div><div><span>Steuer</span><b>${fmt(result.taxes)} €</b></div>${result.open?"<small>Position am Ende offen; darauf noch keine Steuer.</small>":""}</div>`;}).join("")||"<div class='card backtest-empty'>Mindestens eine Strategie aktivieren.</div>";renderBacktestTrades();}
...
for(const input of document.querySelectorAll("#backtestTab input"))input.addEventListener("change",renderBacktest);
$("backtestTradeStrategy").addEventListener("change",renderBacktestTrades);
